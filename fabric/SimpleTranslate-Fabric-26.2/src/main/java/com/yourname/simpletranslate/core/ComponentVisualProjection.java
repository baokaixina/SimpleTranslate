...
public final class ComponentVisualProjection {
    ...
    public List<Integer> atomicGroupSizes() {
        ...
        int currentTopLevel = slots.getFirst().sourceTopLevelIndex();
        int count = 0;
    ...
    /**
     * Wynntils 4.2.2 emits U+00C0 immediately after an AtlasSprite carrier in
     * its content-tracker template. Its own scoreboard/text parsers treat that
     * codepoint as padding, not language. Scope the rule to a standalone run
     * directly following a client-only font marker so an ordinary phrase such
     * as "À la carte" remains translatable everywhere else.
     */
    ...
    private static boolean isCompactCustomFontGlyph(LeafDescriptor leaf) {
        if (leaf != null && ComponentJsonCompat.isLocalFontMarker(leaf.fontId())) {
            ...
            // serializable marker can carry an ordinary letter such as Wynntils'
            // "A" quest icon, but that letter must never reach the model.
            return true;
        }
    ...
    private static void appendRaw(List<Atom> atoms, String text) {
        ...
        if (!atoms.isEmpty() && atoms.getLast().slotIndex() < 0) {
            Atom previous = atoms.removeLast();
