    ...
    public boolean replaceByIdentity(ChatMessageIdentity identity, Component newComponent) {
        ...
        try {
            for (int i = 0; i < allMessages.size(); i++) {
                ...
                if (messageMatchesIdentity(msg, identity)) {
                    allMessages.set(i, new GuiMessage(msg.addedTime(), newComponent, msg.signature(), msg.source(), msg.tag()));
        ...
        try {
            ...
            for (int i = 0; i < indexes.length; i++) {
                ...
                allMessages.set(indexes[i], new GuiMessage(
                        msg.addedTime(), newComponents.get(i), msg.signature(), msg.source(), msg.tag()));
            }
            access.simpleTranslateRescale();
            return true;
        } catch (Exception e) {
