...
public final class ChatTranslationController {
    ...
    public List<String> collectContextLines(Component message, String plainText, int[] targetIndexHolder) {
        ...
        List<GuiMessage<Component>> allMessages = access.simpleTranslateAllMessages();
        ...
        for (int i = 0; i < allMessages.size(); i++) {
            GuiMessage<Component> msg = allMessages.get(i);
        ...
        if (index < 0) {
            for (int i = 0; i < allMessages.size(); i++) {
                GuiMessage<Component> msg = allMessages.get(i);
                Component content = msg.getMessage();
        ...
        for (int i = index - 1; i >= 0 && priorLines.size() < contextLimit; i--) {
            Component content = allMessages.get(i).getMessage();
            if (hudHistoryPresenter.isHudHistoryChatMessage(content)) {
                continue;
            }
            String text = getOriginalContextText(content);
        ...
        try {
            ...
            for (int i = 0; i < allMessages.size(); i++) {
                ...
                if (originalPeer != null
                    ...
                    allMessages.set(i, new GuiMessage(msg.getAddedTime(), originalPeer, msg.getId()));
                ...
                if (blocked) {
                    ...
                    allMessages.set(i, new GuiMessage(msg.getAddedTime(), originalWithButton, msg.getId()));
                    changed = true;
                }
            }
            if (changed) {
