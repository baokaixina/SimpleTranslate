...
public final class VisionOcrTranslationService implements OcrTranslationService {
    ...
    private static final String OCR_ERROR_PREFIX = "\u0000simpletranslate-ocr-error:";
    private static final String OCR_PROBE_PNG_BASE64 =
            "";
    ...
    public CompletableFuture<OcrResult> translateImage(byte[] pngBytes, String imageHash,
        ...
        return TranslationRequestQueue.submit(
                ...
                .thenApply(raw -> raw == null
                        ? OcrResult.failure("OCR request failed after retries")
                        : parseOcrResult(raw));
    }

    @Override
    public CompletableFuture<OcrResult> verifyAccess() {
        return translateImage(Base64.getDecoder().decode(OCR_PROBE_PNG_BASE64),
                "ocr-vision-probe", "auto", ModConfig.TARGET_LANGUAGE.get());
    }

    public CompletableFuture<OcrResult> verifyAccess(boolean reuseTranslationModel,
                                                     ModConfig.ApiFormat format,
                                                     String apiUrl,
                                                     String apiKey,
                                                     String model) {
        ActiveProfile profile = reuseTranslationModel
                ? activeProfileFromConfig(true)
                : activeProfileFromValues(format, apiUrl, apiKey, model);
        if (!profile.ready()) {
            return CompletableFuture.completedFuture(OcrResult.failure(profile.errorMessage()));
        }
        String prompt = buildOcrPrompt("auto", ModConfig.TARGET_LANGUAGE.get());
        String base64 = OCR_PROBE_PNG_BASE64;
        String queueKey = "ocr-probe:" + TranslationCacheKeys.hashSource(profile.queueIdentity());
        return TranslationRequestQueue.submit(
                        queueKey,
                        SURFACE + ".probe",
                        TranslationRequestQueue.Priority.INTERACTIVE,
                        1,
                        () -> sendOcrRequestBlocking(profile, prompt, base64))
                .thenApply(raw -> isValidOcrProbeResponse(raw)
                        ? parseOcrResult(raw)
                        : OcrResult.failure("OCR model did not return the required JSON fields"));
    }
    ...
    public static String buildOcrPrompt(String sourceLanguage, String targetLanguage) {
        ...
        return """
                ...
                {"sourceText":"all source text","translationText":"all translated text","regions":[{"sourceText":"one source block","translationText":"its translation","x":0,"y":0,"width":1000,"height":1000}]}
                regions must contain one entry per visible text line. Do not merge separate lines or distant labels.
                x, y, width, and height are integers normalized to 0..1000 relative to the image.
                Each region must be the tight bounding rectangle of the source glyph pixels, not a loose panel,
                paragraph, translated-text estimate, or surrounding UI container.
                Keep regions in natural reading order. Every translation must correspond only to the source text
                inside that exact region.
                If the image contains no readable text, return {"sourceText":"","translationText":"","regions":[]}.
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                if (looksLikeHtml(response.body())) {
                    return failurePayload("OCR endpoint returned an HTML page; check API URL and interface format");
                }
                String content = extractAssistantContent(response.body());
                return content == null || content.isBlank()
                        ? failurePayload("OCR API returned an empty response")
                        : content;
            }
            ...
            return failurePayload("HTTP " + response.statusCode() + (message.isBlank() ? "" : ": " + message));
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            return failurePayload("OCR request interrupted");
        } catch (IOException e) {
            throw new TranslationRequestQueue.RetryableTranslationException(
            ...
            return failurePayload(e.getMessage() == null ? e.getClass().getSimpleName() : e.getMessage());
        }
    }
    ...
    private ActiveProfile activeProfile() {
        ...
        if (reuse) {
            ActiveProfile translationProfile = activeProfileFromConfig(true);
            if (translationProfile.ready()) {
                return translationProfile;
            }
            ActiveProfile dedicatedProfile = activeProfileFromConfig(false);
            if (dedicatedProfile.ready()) {
                return dedicatedProfile;
            }
            return ActiveProfile.error(translationProfile.format(),
                    translationProfile.errorMessage() + "; OCR dedicated model: " + dedicatedProfile.errorMessage());
        }
        return activeProfileFromConfig(false);
    }

    private ActiveProfile activeProfileFromConfig(boolean reuse) {
        ModConfig.ApiFormat format = reuse ? ModConfig.API_FORMAT.get() : ModConfig.OCR_API_FORMAT.get();
        String url = reuse ? ModConfig.DEEPSEEK_API_URL.get() : ModConfig.OCR_API_URL.get();
        String key = reuse ? ModConfig.DEEPSEEK_API_KEY.get() : ModConfig.OCR_API_KEY.get();
        String model = reuse ? ModConfig.DEEPSEEK_MODEL.get() : ModConfig.OCR_MODEL.get();
        return activeProfileFromValues(format, url, key, model);
    }

    private ActiveProfile activeProfileFromValues(ModConfig.ApiFormat format, String apiUrl,
                                                  String apiKey, String configuredModel) {
        ...
        if (!urlError.isBlank()) {
            return ActiveProfile.error(format, urlError);
        }
        String url = ModConfig.normalizeApiUrl(apiUrl);
        String model = format == ModConfig.ApiFormat.DEEPSEEK_CHAT
                ? normalizedTranslationModel(format, configuredModel)
                : normalizedOcrModel(format, configuredModel);
        if (model.isBlank()) {
    ...
    private static boolean isValidOcrProbeResponse(String raw) {
        if (raw == null || raw.isBlank() || raw.startsWith(OCR_ERROR_PREFIX)) {
            return false;
        }
        String json = objectSlice(stripFences(ModelOutputSanitizer.sanitize(raw)));
        if (json.isBlank()) {
            return false;
        }
        try {
            JsonObject object = JsonParser.parseString(json).getAsJsonObject();
            return object.has("sourceText") && object.has("translationText");
    ...
    private static String extractAssistantContent(JsonObject response) {
        ...
        if (response.has("sourceText") || response.has("translationText")) {
    ...
    private static OcrResult parseOcrResult(String raw) {
        if (raw != null && raw.startsWith(OCR_ERROR_PREFIX)) {
            return OcrResult.failure(raw.substring(OCR_ERROR_PREFIX.length()).trim());
        }
        ...
        if (!json.isBlank()) {
            try {
                ...
                List<OcrRegion> regions = parseRegions(object.get("regions"));
                return OcrResult.success(source, translation, regions);
            } catch (Exception ignored) {
    ...
    private static List<OcrRegion> parseRegions(JsonElement element) {
        ...
        for (JsonElement item : element.getAsJsonArray()) {
            ...
            String source = firstStringField(object, "sourceText", "source", "text");
            String translation = firstStringField(object, "translationText", "translation", "translatedText");
            JsonArray box = firstArrayField(object, "bbox", "box", "bounds");
            int x = coordinate(object, box, 0, "x", "left");
            int y = coordinate(object, box, 1, "y", "top");
            int width = coordinate(object, box, 2, "width", "w");
            int height = coordinate(object, box, 3, "height", "h");
            if (width <= 0 || height <= 0 || translation.isBlank()) {
                continue;
            }
            regions.add(new OcrRegion(source, translation, x, y, width, height).sanitized());
        }
        return regions;
    }

    private static String firstStringField(JsonObject object, String... keys) {
        for (String key : keys) {
    ...
    private static JsonArray firstArrayField(JsonObject object, String... keys) {
        for (String key : keys) {
    ...
    private static int coordinate(JsonObject object, JsonArray box, int boxIndex, String... keys) {
        ...
        if (value == null && box != null && box.size() > boxIndex) {
            value = box.get(boxIndex);
        }
    ...
    private static String failurePayload(String message) {
        String compact = message == null || message.isBlank() ? "OCR request failed" : message.trim();
        compact = compact.replace('\n', ' ').replace('\r', ' ');
        return OCR_ERROR_PREFIX + (compact.length() > 240 ? compact.substring(0, 237) + "..." : compact);
    }
