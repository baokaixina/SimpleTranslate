...
public final class TooltipTranslationGlowRenderer {
    ...
    public static void renderLegacy(GuiGraphics graphics, Font font,
        ...
        int left = mouseX + 12;
        int top = mouseY - 12;
        if (left + width > minecraft.getWindow().getGuiScaledWidth()) {
            left -= 28 + width;
        }
        if (top + height + 6 > minecraft.getWindow().getGuiScaledHeight()) {
            top = minecraft.getWindow().getGuiScaledHeight() - height - 6;
        }

        renderTooltipBounds(graphics, left - 3, top - 3, left + width + 3, top + height + 3, 500.0F);
    }
