...
Write-Host "SimpleTranslate 1.20.1 logic checks (product sync from 1.21.9 donor)"
...
Assert-FileContains "$src/mixin/HoverTooltipMixin.java" 'simple_translate$translateFinalTextComponents' "accepted tooltip rows must be visually replaced at the final renderer"
Assert-FileContains "$src/mixin/HoverTooltipMixin.java" 'ClientTooltipComponent.create(translated.getVisualOrderText())' "tooltip text replacement must preserve non-text overlay components"
Assert-FileContains "$src/mixin/HoverTooltipMixin.java" 'overlayTooltipSubmission' "direct overlay replays must own an item-tooltip frame"
...
Assert-FileContains "$src/mixin/compat/BetterAdvancementsWidgetTranslationMixin.java" 'drawHover(Lnet/minecraft/class_332;IIFII)V' "Better Advancements exact 0.4.2.10 hover descriptor"
Assert-FileContains "$src/mixin/compat/AdvancementPlaqueTranslationMixin.java" 'drawPlaque(Lnet/minecraft/class_332;J)Lnet/minecraft/class_368$class_369;' "Advancement Plaques exact 1.6.7 renderer descriptor"
Assert-FileContains "$src/mixin/compat/AdvancementPlaqueTranslationMixin.java" 'require = 3' "Advancement Plaques must intercept all three direct Font Component draws"
Assert-FileContains "$res/simple_translate.mixins.json" "BetterAdvancementsWidgetTranslationMixin" "Better Advancements optional mixin registered"
Assert-FileContains "$res/simple_translate.mixins.json" "AdvancementPlaqueTranslationMixin" "Advancement Plaques optional mixin registered"
...
$retiredIcebergGatherClass = "IcebergTooltip" + "GatherCompat"
$retiredGatherMethod = "translateGathered" + "TooltipLines"
Assert-PathMissing "$src/compat/$retiredIcebergGatherClass.java" "early Iceberg gather translation competes with the final tooltip renderer"
Assert-FileNotContains "$src/SimpleTranslateMod.java" $retiredIcebergGatherClass "Iceberg gather listener must stay unregistered"
Assert-FileNotContains "$src/feature/tooltip/TooltipTranslationHelper.java" $retiredGatherMethod "retired gather-stage tooltip path must be deleted"
...
Assert-FileContains "$src/mixin/SimpleTranslateMixinPlugin.java" 'ADVANCEMENT_PLAQUES_MOD_ID = "advancementplaques"' "Advancement Plaques optional mixin gate"
Assert-FileContains "$src/mixin/SimpleTranslateMixinPlugin.java" 'BETTER_ADVANCEMENTS_MOD_ID = "betteradvancements"' "Better Advancements optional mixin gate"
...
Write-Host "1.20.1 logic checks PASSED"
