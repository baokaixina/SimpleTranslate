param(
    ...
    [switch]$PreserveClientDependencies,

    [string[]]$IgnoredFatalLogLinePattern = @(),
...
function Remove-IgnoredFatalLogLines {
    param(
        [string]$Text,
        [string[]]$Patterns
    )

    if ([string]::IsNullOrWhiteSpace($Text) -or -not $Patterns -or $Patterns.Count -eq 0) {
        return $Text
    }
...
function Invoke-EnterSingleplayerWorldUi {
    ...
    # a real user save.
    Invoke-WindowClick -Handle $handle -XRatio 0.50 -YRatio 0.16
    Start-Sleep -Milliseconds 500
...
$deployedDependencies = if ($PreserveClientDependencies) {
    @()
} else {
    Deploy-TestClientDependencies -ProjectPath $project -Mapping $mapping -ModsDir $modsDir
}
...
while (((Get-Date) - $startTime).TotalSeconds -lt $TimeoutSeconds) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($combinedLaunchOutput)) {
        $combinedLaunchOutput = Remove-IgnoredFatalLogLines `
            -Text $combinedLaunchOutput -Patterns $IgnoredFatalLogLinePattern
    ...
    if (Test-Path -LiteralPath $latestLog) {
        ...
        $fatalScanLog = Remove-IgnoredFatalLogLines `
            -Text $lastLog -Patterns $IgnoredFatalLogLinePattern

        foreach ($pattern in $fatalPatterns) {
            if ($fatalScanLog -match $pattern) {
