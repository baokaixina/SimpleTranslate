...
public final class HudTranslationHistory {
    ...
    public static final long DEFAULT_BATCH_INTERVAL_MS = 1_500L;
    ...
    private static List<String> inFlightHistoryKeys = List.of();
    ...
    private static BatchSnapshot prepareNextBatchLocked(long nowMillis) {
        if (inFlightBatchId >= 0L || nowMillis < nextBatchAllowedAtMillis) {
            return null;
        }
        ...
        inFlightHistoryKeys = List.copyOf(historyKeys);
    ...
    private static void completeBatchLocked(long batchId, List<Component> translatedComponents, long nowMillis) {
        if (batchId != inFlightBatchId || translatedComponents == null
                || translatedComponents.size() != inFlightHistoryKeys.size()) {
            return;
        }
        for (int i = 0; i < inFlightHistoryKeys.size(); i++) {
            EntryState entry = findEntry(inFlightHistoryKeys.get(i));
            if (entry != null) {
    ...
    private static void failBatch(long batchId, long nowMillis) {
        synchronized (HudTranslationHistory.class) {
            ...
            for (String key : inFlightHistoryKeys) {
    ...
    private static List<Entry> readyChatEntriesLocked() {
        if (!ModConfig.HUD_HISTORY_CHAT_ENABLED.get()) {
            return List.of();
        }
    ...
    private static String buildBatchContextBefore(long firstBatchSequence) {
        ...
        context.append("The current batch lines are consecutive subtitle fragments in <st-plain-context>; ");
