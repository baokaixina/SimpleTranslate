...
Write-Host "SimpleTranslate 1.20 logic checks (product sync from 1.21.9 donor)"
...
Assert-PathMissing "$src/feature/wynn" "Wynn package must not ship on Minecraft 1.20"
...
Assert-PathMissing "$src/mixin/compat/WynntilsOverlayManagerMixin.java" "Wynntils compat mixin must not ship on 1.20"
...
Assert-FileNotContains "$src/config/ModConfig.java" "HUD_WYNN_OVERLAY_ENABLED" "Wynn overlay switch must not ship on 1.20"
...
Assert-FileNotContains "$res/assets/simple_translate/lang/zh_cn.json" "hud.wynn" "zh Wynn HUD keys must not ship on 1.20"
Assert-FileNotContains "$res/assets/simple_translate/lang/en_us.json" "hud.wynn" "en Wynn HUD keys must not ship on 1.20"
...
Assert-FileNotContains "$src/feature/hud/HudFeature.java" "Wynn" "HudFeature must carry no Wynn layer on 1.20"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "Wynn" "GUI helper must carry no Wynn overlay layer on 1.20"
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "DialogScreen" "1.20 has no dialog screen API"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "hud.wynn" "no Wynn overlay cache surface may ship on 1.20"
...
Assert-FileNotContains "$src/core/JsonPassthroughPipeline.java" ".wynn." "pipeline must carry no Wynn recovery branches on 1.20"
...
Assert-FileNotContains "$src/transport/JsonPassthroughPrompts.java" "Wynncraft" "prompts must carry no Wynn branches on 1.20"
...
Assert-FileContains "$src/mixin/ScreenGuiTranslationMixin.java" "renderWithTooltip(" "1.20 GUI frame scope uses renderWithTooltip"
...
Assert-FileContains "$src/mixin/HoverTooltipMixin.java" "renderTooltipInternal" "1.20 tooltips funnel through renderTooltipInternal"
...
Assert-FileNotContains "$src/mixin/TitleOverlayMixin.java" "Wynn" "title overlay must carry no Wynn hooks on 1.20"
...
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" '"render(Lnet/minecraft/client/gui/GuiGraphics;F)V"' "1.20 HUD frame must hang on the exact Gui.render owner"
...
Assert-FileNotContains "$src/mixin/TextDisplayMixin.java" "wynn" "text display must carry no Wynn label surface on 1.20"
...
Assert-FileContains "$src/keybind/ModKeyBindings.java" '"key.category.simple_translate.general"' "1.20 KeyMapping category is a plain translation-key string"
...
Assert-FileContains "$src/cache/SharedCachePayload.java" 'new ResourceLocation("simple_translate", "cache_sync/v1")' "1.20 shared cache uses a classic channel"
Assert-FileNotContains "$src/cache/SharedCachePayload.java" "CustomPacketPayload" "1.20 has no CustomPacketPayload API"
...
Write-Host "1.20 logic checks PASSED"
