...
public final class HudHistoryChatPresenter {
    ...
    public void upsertHudHistoryCaption(HudTranslationHistory.Entry entry) {
        ...
        if (!controller.replacer().replaceByClickValue(clickValue, message, 120)) {
            ...
            controller.access().simpleTranslateAllMessages().add(0, new GuiMessage(ticks, message, 0));
        }
    ...
    private Component createHudHistoryChatMessage(HudHistoryChatData data) {
        ...
        MutableComponent content = com.yourname.simpletranslate.core.LegacyComponentFactory.empty()
                .append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(entry.type().label() + " ").withStyle(typeColor))
                .append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(bodyText).withStyle(ChatFormatting.WHITE));
    ...
    private void trimHudHistoryChatMessages() {
        List<GuiMessage<Component>> allMessages = controller.access().simpleTranslateAllMessages();
