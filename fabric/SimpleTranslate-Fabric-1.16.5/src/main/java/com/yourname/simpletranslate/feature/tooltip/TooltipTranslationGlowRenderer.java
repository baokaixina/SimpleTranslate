...
public final class TooltipTranslationGlowRenderer {
    ...
    public static void render(GuiGraphics graphics, Screen screen, Font font,
                              List<? extends FormattedCharSequence> components,
                              int mouseX, int mouseY) {
        if (graphics == null || font == null || components == null || components.isEmpty()
                || screen == null || !ModConfig.TOOLTIP_GLOW_ENABLED.get()) {
            return;
        }

        int width = 0;
        int height = components.size() == 1 ? -2 : 0;
        for (FormattedCharSequence component : components) {
        ...
        // 1.19.2 has no ClientTooltipPositioner; renderTooltipInternal computes
        // the tooltip origin inline with this exact clamping.
        int left = mouseX + 12;
        if (left + width > screen.width) {
            left -= 28 + width;
        }
        int top = mouseY - 12;
        if (top + height + 6 > screen.height) {
            top = screen.height - height - 6;
        }
        renderTooltipBounds(graphics, left - 3, top - 3,
                left + width + 3, top + height + 3, true);
    }
