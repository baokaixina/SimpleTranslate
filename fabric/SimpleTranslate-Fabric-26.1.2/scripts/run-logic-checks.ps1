...
try {
    ...
    Assert-FileContains $jsonPipeline "array.size() != expected" "JSON responses must keep top-level count"
    Assert-FileContains $jsonPipeline "ComponentJsonCompat.fromJson" "JSON responses must parse as Components"
    ...
    Assert-FileContains $signRendererMixin "renderer/state/level/CameraRenderState" "26.x sign selection outline must target the relocated camera render state"
