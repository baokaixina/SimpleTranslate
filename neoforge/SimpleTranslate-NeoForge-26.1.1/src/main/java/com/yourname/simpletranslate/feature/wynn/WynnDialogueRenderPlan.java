...
public final class WynnDialogueRenderPlan {
    ...
    public boolean render(GuiGraphicsExtractor graphics, Font font,
        ...
        for (PositionedLine line : metrics.layout().lines()) {
            ...
            try {
                graphics.pose().translate(x + line.x(), y + line.y());
                graphics.text(font, line.text(), 0, 0, color, true);
            } finally {
    ...
    private static void drawUnmaskedSourceRuns(
            GuiGraphicsExtractor graphics, Font font, List<SourceReplayRun> sourceReplay,
            int x, int y, int color) {
        for (SourceReplayRun run : sourceReplay) {
            graphics.pose().pushMatrix();
            try {
                graphics.pose().translate(x + run.x(), y);
                graphics.text(font, run.sequence(), 0, 0, color, true);
            } finally {
                graphics.pose().popMatrix();
            }
        }
    ...
    private static Bounds bounds(Font font, FormattedCharSequence sequence, boolean shadow) {
        Font.PreparedText prepared = font.prepareText(sequence, 0.0F, 0.0F,
                0xFFFFFFFF, shadow, false, 0);
        ScreenRectangle rectangle = prepared.bounds();
