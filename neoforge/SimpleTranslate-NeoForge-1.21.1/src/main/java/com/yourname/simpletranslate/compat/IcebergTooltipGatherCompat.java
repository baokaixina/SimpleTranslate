 ...
 *
 * <p>API surface verified with javap against Iceberg-1.21-neoforge-1.2.5
 * (Minecraft 1.21) and Iceberg-1.21.1-neoforge-1.3.2 (Minecraft 1.21.1): the
 * event class is {@code com.anthonyhilyard.iceberg.events.client.RenderTooltipEvents}
 * and its {@code GATHER} is Iceberg's own public
 * {@code com.anthonyhilyard.iceberg.events.Event} with a public register(T);
 * Gather.onGather takes (ItemStack, int, int, List, int, int) and GatherResult
 * has a public (InteractionResult, int, List) constructor in both jars. The actual translation runs through
