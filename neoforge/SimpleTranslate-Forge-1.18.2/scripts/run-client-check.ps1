...
function Ensure-User32 {
    if ([type]::GetType("CodexUser32", $false)) {
        Enable-PerMonitorDpiCoordinates
        return
    }
    Add-Type @"
using System;
...
function Enable-PerMonitorDpiCoordinates {
    # Use one physical-pixel coordinate space for GetClientRect,
    # ClientToScreen, SetCursorPos and screenshot capture. Without this,
    # a PowerShell host that Windows treats as DPI-unaware can receive a
    # virtualized window rectangle but a non-virtualized client origin; the
    # resulting click is shifted by a menu row (1.18.2 hit Multiplayer).
    try {
        [CodexUser32]::SetThreadDpiAwarenessContext([IntPtr](-4)) | Out-Null
    } catch {
        # Older Windows builds may not provide the per-monitor-v2 context.
        # The original coordinate behavior remains the safe fallback there.
    }
}
...
function Get-BuiltModJar {
    ...
    # ForgeGradle 6 jarJar produces a "-all" fat jar; on Forge targets it is the
    # only deployable artifact because it embeds MixinExtras (absent from Forge,
    # unlike Fabric Loader) alongside the filled searge refmap. Prefer it when
    # present, otherwise fall back to the slim jar selection used by Fabric targets.
    $fatJars = Get-ChildItem -LiteralPath $libs -Filter "*-all.jar" -File |
        Where-Object { $_.Name -notmatch "(-sources|-javadoc)\.jar$" } |
        Sort-Object LastWriteTime -Descending
    if ($fatJars.Count -gt 0) {
        return $fatJars[0]
    }
...
function Ensure-ExactMinecraftLibraryFile {
    param(
        $Download,
        [string]$LibrariesRoot
    )

    if ($null -eq $Download -or [string]::IsNullOrWhiteSpace([string]$Download.path)) {
        return
    }

    $destination = Join-Path $LibrariesRoot ([string]$Download.path)
    $expectedSha1 = [string]$Download.sha1
    if (Test-Path -LiteralPath $destination -PathType Leaf) {
        if (-not [string]::IsNullOrWhiteSpace($expectedSha1)) {
            ...
            if (-not [string]::Equals($actualSha1, $expectedSha1.ToLowerInvariant(), [System.StringComparison]::Ordinal)) {
                throw "Existing test-client library has the wrong SHA-1: $destination"
            }
        }
        return
    }

    if ([string]::IsNullOrWhiteSpace([string]$Download.url)) {
        throw "Missing download URL for required test-client library: $($Download.path)"
    }

    New-Item -ItemType Directory -Path (Split-Path -Parent $destination) -Force | Out-Null
    $temporary = "$destination.codex-download"
    try {
        Invoke-WebRequest -Uri ([string]$Download.url) -OutFile $temporary
        if (-not [string]::IsNullOrWhiteSpace($expectedSha1)) {
            $actualSha1 = (Get-FileHash -LiteralPath $temporary -Algorithm SHA1).Hash.ToLowerInvariant()
            if (-not [string]::Equals($actualSha1, $expectedSha1.ToLowerInvariant(), [System.StringComparison]::Ordinal)) {
                throw "Downloaded test-client library failed SHA-1 verification: $($Download.path)"
            }
        }
        Move-Item -LiteralPath $temporary -Destination $destination
        Write-Output "INFO: provisioned exact Minecraft library $($Download.path)"
    } finally {
        Remove-Item -LiteralPath $temporary -Force -ErrorAction SilentlyContinue
    }
}

function Ensure-ExactMinecraftLibraries {
    param(
        $VersionJson,
        [string]$LibrariesRoot
    )
    ...
    foreach ($lib in @($VersionJson.libraries)) {
        ...
        Ensure-ExactMinecraftLibraryFile -Download $lib.downloads.artifact -LibrariesRoot $LibrariesRoot
        ...
        if ($lib.natives -and $lib.natives.windows -and $lib.downloads.classifiers) {
            ...
            $nativeDownload = $lib.downloads.classifiers.$classifier
            Ensure-ExactMinecraftLibraryFile -Download $nativeDownload -LibrariesRoot $LibrariesRoot
        }
    }
}
...
function Resolve-VersionGameJar {
    param(
        [string]$RealVersionDir,
        [string]$VersionJsonPath
    )

    # Forge launcher profiles commonly inherit the vanilla client jar rather
    # than copying it into the Forge version directory. Follow the unmerged
    # profile chain so array-valued helper metadata cannot be collapsed by
    # PowerShell parameter binding.
    $versionsRoot = Split-Path -Parent $RealVersionDir
    $currentJsonPath = $VersionJsonPath
    $seenIds = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
    $checked = New-Object System.Collections.Generic.List[string]
    while (-not [string]::IsNullOrWhiteSpace($currentJsonPath)) {
        if (-not (Test-Path -LiteralPath $currentJsonPath -PathType Leaf)) {
            throw "Missing inherited version json while resolving client game jar: $currentJsonPath"
        }
        $profile = Get-Content -LiteralPath $currentJsonPath -Raw | ConvertFrom-Json
        $id = [string]$profile.id
        if ([string]::IsNullOrWhiteSpace($id) -or -not $seenIds.Add($id)) {
            throw "Invalid or cyclic inherited version chain while resolving client game jar: $($checked -join ' -> ')"
        }
        $checked.Add($id)
        $candidate = Join-Path $versionsRoot (Join-Path $id "$id.jar")
        if (Test-Path -LiteralPath $candidate -PathType Leaf) {
            return $candidate
        }
        $parentId = [string]$profile.inheritsFrom
        if ([string]::IsNullOrWhiteSpace($parentId)) {
            break
        }
        $currentJsonPath = Join-Path $versionsRoot (Join-Path $parentId "$parentId.json")
    }

    throw "Missing test-client game jar for version chain: $($checked -join ' -> ')"
}
...
Ensure-ExactMinecraftLibraries -VersionJson $versionJson -LibrariesRoot $librariesRoot
Expand-NativeLibraries -VersionJson $versionJson -LibrariesRoot $librariesRoot -NativesDir $nativesDir
$realVersionJar = Resolve-VersionGameJar -RealVersionDir $realVersionDir -VersionJsonPath $versionJsonPath
