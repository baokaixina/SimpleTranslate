...
public class CacheEditScreen extends BaseSimpleTranslateScreen {
    ...
    protected void init() {
        ...
        this.translationInput = new MultiLineEditBox.Builder()
                .setX(left)
                .setY(inputY)
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font, Component.translatable("screen.simple_translate.cache.edit.translation"),
                left, INPUT_TOP - 13, 0xFFFFFFFF);
        if (status != null && !status.getString().isBlank()) {
            graphics.centeredText(this.font, status, this.width / 2, this.height - 42, 0xFF88FF88);
        }
    ...
    private void drawSourcePanel(GuiGraphicsExtractor graphics, int left, int top, int width, int height) {
        graphics.text(this.font, Component.translatable("screen.simple_translate.cache.edit.source"),
        ...
        for (int i = 0; i < Math.min(lines.size(), maxLines); i++) {
            graphics.text(this.font, lines.get(i), left + 4, y, 0xFFDDDDDD);
            y += this.font.lineHeight;
        }
        if (lines.size() > maxLines) {
            graphics.text(this.font, "...", left + width - 16, top + height - this.font.lineHeight - 2, 0xFFAAAAAA);
        }
    }
    ...
    private void drawTranslationPreview(GuiGraphicsExtractor graphics, int left, int top, int width, int height) {
        graphics.text(this.font, Component.translatable("screen.simple_translate.cache.edit.preview"),
                left, top - 12, 0xFFFFFFFF);
        graphics.fill(left - 1, top - 1, left + width + 1, top + height + 1, 0xAA000000);
        ...
        for (int i = previewScrollLine; i < end; i++) {
            graphics.text(this.font, visualLines.get(i), left + 4, y, 0xFFFFFFFF);
            y += this.font.lineHeight;
        }
    ...
    private void drawPreviewScrollbar(GuiGraphicsExtractor graphics, int left, int top, int width, int height,
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        if (overPreview && previewMaxScroll > 0 && scrollY != 0.0D) {
            int amount = Math.max(1, (int) Math.ceil(Math.abs(scrollY) * 3.0D));
            previewScrollLine += scrollY > 0.0D ? -amount : amount;
            previewScrollLine = Math.max(0, Math.min(previewScrollLine, previewMaxScroll));
            return true;
        }
