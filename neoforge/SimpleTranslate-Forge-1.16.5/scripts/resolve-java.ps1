function Use-SimpleTranslateProjectJava {
    ...
    # Java 8 reports `1.8.0_x`; Java 9+ reports the real feature version.
    # Capture the legacy prefix separately so both forms compare against the
    # project-declared major version correctly.
    $versionMatch = [regex]::Match($versionText, '(?:version\s+")?(?:(?<legacy>1)\.)?(?<major>\d+)(?:\.|\")')
