...
public final class SignContextSelectionManager {
    ...
    private static void addLookedAtSign(boolean quietMiss) {
        Minecraft minecraft = Minecraft.getInstance();
        if (!(minecraft.hitResult instanceof BlockRayTraceResult hitResult)
                || hitResult.getType() != RayTraceResult.Type.BLOCK
        ...
        if (!(minecraft.level.getBlockEntity(pos) instanceof SignTileEntity sign)) {
            if (!quietMiss) {
                showActionbar("message.simple_translate.sign_context.no_target");
            }
            return;
    ...
    public static List<SelectionView> getRenderableSelections(World level) {
        if (level == null) {
            return List.of();
        }
    ...
    private static void pruneStaleSelections(World level) {
        String worldKey = currentWorldKey();
        String dimensionKey = currentDimensionKey(level);
        SELECTIONS.entrySet().removeIf(entry -> {
            ...
            if (!(level.getBlockEntity(selection.pos) instanceof SignTileEntity sign)) {
                return true;
            }
    ...
    private static String createKey(World level, BlockPos pos, boolean front, String[] lines) {
    ...
    private static String currentDimensionKey(World level) {
    ...
    private static void showCountMessage(String key) {
        ...
        if (minecraft.player != null) {
            minecraft.player.displayClientMessage(com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(key, SELECTIONS.size()), true);
        }
    }
    ...
    private static void showActionbar(String key) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.player != null) {
            minecraft.player.displayClientMessage(com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(key), true);
        }
    }
