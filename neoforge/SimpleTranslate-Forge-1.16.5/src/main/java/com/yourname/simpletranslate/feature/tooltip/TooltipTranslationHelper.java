...
public final class TooltipTranslationHelper {
    ...
    private static final IdentityMarker<List<ITextComponent>> TRANSLATED_COMPONENT_LISTS =
            new IdentityMarker<>(MAX_TRANSLATED_SIGNATURES);
    private static final IdentityMarker<ITextComponent> TRANSLATED_COMPONENTS =
            new IdentityMarker<>(MAX_TRANSLATED_SIGNATURES);
    ...
    public static boolean isTranslationPending(List<ITextComponent> components,
    ...
    public static boolean isHoverTranslationPending(ITextComponent component) {
        if (component == null) {
            return false;
        }
        List<ITextComponent> lines = splitComponentByNewlines(component);
    ...
    private static void markActiveTranslationGlow(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return;
        }
    ...
    private static void clearActiveTranslationGlow(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return;
        }
    ...
    private static boolean isActiveTranslationGlow(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return false;
        }
    ...
    public static List<ITextComponent> splitHoverComponentLinesForRender(ITextComponent component) {
        if (component == null) {
            return List.of();
        }
    ...
    public static List<ITextComponent> splitHoverComponentsLinesForRender(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
        ...
        List<ITextComponent> wrapped = wrapPendingHoverLines(lines);
        return wrapped.size() != sourceSize || !wrapped.equals(components) ? wrapped : components;
    }

    private static List<ITextComponent> wrapPendingHoverLines(List<ITextComponent> lines) {
        ...
        List<ITextComponent> wrapped = new ArrayList<>();
        ...
        for (ITextComponent line : lines) {
            ...
            List<ITextComponent> split = wrapStyledTooltipComponent(line, maxWidth, font);
    ...
    public static ITextComponent translateComponentWithStyle(ITextComponent component) {
    ...
    public static List<ITextComponent> translateHoverComponentLines(ITextComponent component) {
        return translateHoverComponentLines(component, true);
    }

    private static ITextComponent lastHoverLinesSource;
    private static boolean lastHoverLinesAllowRequest;
    private static long lastHoverLinesRevision = -1L;
    private static List<ITextComponent> lastHoverLinesResult;

    public static List<ITextComponent> translateHoverComponentLines(ITextComponent component, boolean allowRequest) {
        if (component != null) {
            ...
            List<ITextComponent> result = translateHoverComponentLinesInner(component, allowRequest);
    ...
    private static List<ITextComponent> translateHoverComponentLinesInner(ITextComponent component, boolean allowRequest) {
        List<ITextComponent> fallback = component == null ? List.of() : List.of(component);
        return SafeTranslate.guard(() -> {
            if (component == null) {
                return List.of();
            }
            ...
            List<ITextComponent> lines = splitComponentByNewlines(component);
            List<ITextComponent> translated = translateSemanticProjection(
    ...
    public static ITextComponent translateComponent(ITextComponent component) {
        if (component == null || isMarkedTranslatedTooltip(component) || !containsEnglish(component.getString())) {
            return component;
        }
    ...
    private static ITextComponent translateComponentImpl(ITextComponent component) {
        ComponentTranslationResult direct =
    ...
    public static boolean anyContainsEnglish(List<ITextComponent> components) {
        if (components == null) {
            return false;
        }
        for (ITextComponent component : components) {
    ...
    private static boolean hasVisibleTextChange(ITextComponent source, ITextComponent translated) {
        if (translated == null) {
            return false;
        }
        if (source == null) {
    ...
    private static boolean hasVisibleTextChange(List<ITextComponent> source, List<ITextComponent> translated) {
        if (translated == null) {
            return false;
        }
        if (source == null) {
    ...
    private static String normalizedVisibleText(List<ITextComponent> components) {
    ...
    public static boolean isMarkedTranslatedTooltip(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return false;
        }
    ...
    public static boolean isMarkedTranslatedTooltip(ITextComponent component) {
        if (component == null) {
            return false;
        }
    ...
    public static void markTranslatedTooltip(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return;
        }
    ...
    public static void markTranslatedTooltip(ITextComponent component) {
        if (component == null) {
            return;
        }
     ...
     */
    public static List<ITextComponent> translateGatheredTooltipLines(
    ...
    private static List<ITextComponent> translateSemanticProjection(
            List<ITextComponent> components,
            TooltipTranslationController.RenderContext context,
            boolean allowRequest) {
        ComponentVisualProjection projection = JsonPassthroughPipeline.projectLiveComponents(
        ...
        List<ITextComponent> semantic = projection.semanticComponents();
        ...
        if (ready != null) {
            if (ready.size() == projection.slotCount()) {
                List<ITextComponent> rebuilt = rebuildSemanticResult(projection, ready, components);
                if (rebuilt != null) {
        ...
        if (cached.translated && cached.components != null && !cached.components.isEmpty()) {
            List<ITextComponent> readySemantic = translatedSemanticComponents(projection, cached.components);
            ...
            List<ITextComponent> translated = constrainTranslatedTooltipLines(cached.components, components);
        ...
        List<ITextComponent> legacy = tryLegacySemanticCacheCandidate(
        ...
        if (delta.fullyResolved()) {
            List<ITextComponent> readySemantic = delta.resolvedComponents();
            TooltipSemanticResultStore.put(readyKey, readySemantic);
            List<ITextComponent> cacheable = projection.rebuildComponentList(readySemantic);
            if (cacheable != null && !cacheable.isEmpty()) {
                JsonPassthroughPipeline.cacheResolvedComponents(
                        components, cacheable, surface, SEMANTIC_ROLE, stableContext,
                        SimpleTranslateMod.getRuntimeRevision());
            }
            List<ITextComponent> rebuilt = rebuildSemanticResult(projection, readySemantic, components);
            if (rebuilt != null) {
                clearActiveTranslationGlow(components);
                return rebuilt;
            }
    ...
    private static List<ITextComponent> tryLegacySemanticCacheCandidate(
            List<ITextComponent> components, ComponentVisualProjection projection,
    ...
    private static List<ITextComponent> tryLegacySemanticCacheCandidate(
            TranslationCache cache, List<ITextComponent> components, ComponentVisualProjection projection,
            String surface, String readyKey, String reuseScope) {
        if (cache == null || projection == null || !projection.hasSlots()) {
            return null;
        }
        String sourceText = projection.semanticComponents().stream()
                .map(ITextComponent::getString).reduce("", (left, right) ->
        ...
        for (TranslationCache.SemanticCacheCandidate candidate
            ...
            List<ITextComponent> semantic = parseExactSemanticCandidate(
                    candidate.payload(), projection.slotCount());
            if (semantic == null) {
                continue;
            }
            List<ITextComponent> rebuilt = projection.rebuildComponentList(semantic);
    ...
    private static List<ITextComponent> parseExactSemanticCandidate(String payload, int expected) {
        if (payload == null || payload.isBlank() || expected <= 0) {
            return null;
        }
        try {
            JsonElement root = new com.google.gson.JsonParser().parse(payload);
            if (!root.isJsonArray() || root.getAsJsonArray().size() != expected) {
                return null;
            }
            List<ITextComponent> parsed = new ArrayList<>(expected);
    ...
    private static void scheduleSemanticRefresh(List<ITextComponent> source,
        ...
        List<ITextComponent> sourceSnapshot = List.copyOf(source);
        ...
        DirectSurfaceTranslator.translateComponentsAsync(
                ...
                .whenComplete((result, error) -> {
                    ...
                    List<ITextComponent> readySemantic = mergeSemanticDelta(
    ...
    private static SemanticDeltaPlan planSemanticDelta(
        ...
        List<String> sources = projection.slots().stream()
                ...
                .toList();
        List<ITextComponent> semantic = projection.semanticComponents();
        List<ITextComponent> resolved = new ArrayList<>(Collections.nCopies(sources.size(), null));
        List<Integer> missingIndexes = new ArrayList<>();
        List<ITextComponent> missingComponents = new ArrayList<>();
        ...
        // Incremental reuse is atomic per original top-level ITextComponent, not per
    ...
    private static List<ITextComponent> mergeSemanticDelta(
            SemanticDeltaPlan delta, List<ITextComponent> requested,
        ...
        List<ITextComponent> merged = new ArrayList<>(delta.resolved());
        for (int index = 0; index < delta.missingIndexes().size(); index++) {
            ITextComponent component = requested.get(index);
            if (component == null) {
                return null;
            }
    ...
    private static void recordScopedSemanticTranslations(
            ComponentVisualProjection projection, List<ITextComponent> translatedSemantic,
    ...
    private static String semanticDeltaRequestContext(
        ...
        if (delta != null) {
            ...
            for (int index = 0; index < delta.sourceTexts().size() && appended < 16; index++) {
                ITextComponent translated = delta.resolved().get(index);
    ...
    private static List<ITextComponent> rebuildSemanticResult(
            ComponentVisualProjection projection, List<ITextComponent> semantic,
            List<ITextComponent> originals) {
        List<ITextComponent> rebuilt = projection == null ? null
    ...
    private record SemanticDeltaPlan(
            ...
            List<ITextComponent> resolved,
            List<Integer> missingIndexes,
            List<ITextComponent> missingComponents,
        ...
        private List<ITextComponent> resolvedComponents() {
    ...
    private static String semanticContext(TooltipTranslationController.RenderContext context,
                                          List<ITextComponent> sourceComponents) {
    ...
    private static List<ITextComponent> translatedSemanticComponents(
            ComponentVisualProjection projection, List<ITextComponent> restored) {
        ...
        try {
            JsonElement restoredRoot = new com.google.gson.JsonParser().parse(restoredJson);
            ...
            return translated.stream().map(text -> (ITextComponent) com.yourname.simpletranslate.core.LegacyComponentFactory.literal(text)).toList();
        } catch (RuntimeException ignored) {
            return null;
        }
    }

    public static String buildOverlayTooltipContext(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return "";
        }
    ...
    public static String buildHoverTooltipContext(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return "";
        }
        StringBuilder context = new StringBuilder();
    ...
    private static String tooltipSignature(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return "";
        }
    ...
    private static String tooltipSemanticSignature(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return "";
        }
        StringBuilder signature = new StringBuilder();
    ...
    private static String componentSignature(ITextComponent component) {
        if (component == null) {
            return "<null>";
        }
    ...
    private static List<ITextComponent> constrainTranslatedTooltipLines(List<ITextComponent> translated, List<ITextComponent> originals) {
        if (translated == null || translated.isEmpty()) {
            return translated;
        }
        Minecraft minecraft = Minecraft.getInstance();
        ...
        List<ITextComponent> wrapped = new ArrayList<>();
        boolean changed = false;
        for (ITextComponent component : translated) {
            if (component == null || font.width(component) <= maxWidth) {
                wrapped.add(component);
                continue;
            }
            List<ITextComponent> split = wrapStyledTooltipComponent(component, maxWidth, font);
            wrapped.addAll(split);
    ...
    private static int translatedTooltipMaxWidth(List<ITextComponent> originals) {
    ...
    private static int getMaxTooltipLineWidth(List<ITextComponent> components) {
        Minecraft minecraft = Minecraft.getInstance();
        FontRenderer font = minecraft == null ? null : minecraft.font;
    ...
    private static List<ITextComponent> wrapStyledTooltipComponent(ITextComponent component, int maxWidth, FontRenderer font) {
        if (component == null) {
            return List.of();
        }
        ...
        try {
            ...
            IFormattableTextComponent current = com.yourname.simpletranslate.core.LegacyComponentFactory.empty();
            ...
            for (TextSegmentInfo segment : segments) {
                ...
                for (int offset = 0; offset < segment.text.length();) {
                    ...
                    float pieceWidth = font.getSplitter().stringWidth(ITextProperties.of(piece, style));
    ...
    private static List<ITextComponent> splitComponentByNewlines(ITextComponent component) {
        List<TextSegmentInfo> segments = new ArrayList<>();
        ComponentSegmentHelper.extractSegments(component, segments, Style.EMPTY, true);
        List<ITextComponent> lines = new ArrayList<>();
        IFormattableTextComponent current = com.yourname.simpletranslate.core.LegacyComponentFactory.empty();
