...
public final class SharedCacheServer {
    private static final Logger LOGGER = LogManager.getLogger("SimpleTranslateSharedCache");
    ...
    private static void onServerStarted(FMLServerStartedEvent event) {
        ...
        Path storeFile = server.getWorldPath(FolderName.ROOT).resolve(STORE_FILE);
    ...
    private static void onServerTick(TickEvent.ServerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            flushOutboundQueues(ServerLifecycleHooks.getCurrentServer());
    ...
    public static void handlePayload(ServerPlayerEntity player, SharedCachePayload payload) {
        MinecraftServer server = player == null ? null : player.server;
        handlePayload(server, player, payload);
    }

    private static void handlePayload(MinecraftServer server, ServerPlayerEntity player, SharedCachePayload payload) {
    ...
    private static void broadcastEntries(MinecraftServer server, ServerPlayerEntity source, List<SharedCacheEntry> entries) {
    ...
    private static boolean canSend(ServerPlayerEntity player) {
        return SharedCacheNetworking.canSendToPlayer(player);
    }

    private static void sendPayload(ServerPlayerEntity player, SharedCachePayload payload) {
