...
/**
 * Scoreboard text uses one ordered ITextComponent-array job per visible frame. Each
...
public final class ScoreboardTranslationHelper {
     ...
     */
    public static ITextComponent translateFrameComponent(ITextComponent component) {
     ...
     * right-aligned scores must measure the same ITextComponent that is rendered.
     */
    public static ITextComponent translateKnownComponent(ITextComponent component) {
        ...
        Map<String, ITextComponent> known = knownTranslationsByKey(snapshot);
        ITextComponent translated = known.get(componentKey(component));
        return translated == null ? component : translated;
    }

    private static FrameSnapshot knownMapSnapshot;
    private static Map<String, ITextComponent> knownMap;

    private static Map<String, ITextComponent> knownTranslationsByKey(FrameSnapshot snapshot) {
        if (snapshot != knownMapSnapshot) {
            Map<String, ITextComponent> map = new HashMap<>();
            for (int index = 0; index < snapshot.sourceKeys.size(); index++) {
                ITextComponent translated = snapshot.translated.get(index);
                if (translated != null) {
    ...
    public static void endFrame() {
        ...
        List<ITextComponent> sources = List.copyOf(frame.sources);
        ...
        if (result != null && result.translated && result.components != null
            ...
            List<ITextComponent> mapped = projection.bind(result.components);
    ...
    private static ITextComponent lastListSource;
    private static ITextComponent lastListResult;
    private static long nextListRetryAtNanos;

    public static ITextComponent translateListComponent(ITextComponent component) {
        if (component == null || !ModConfig.GLOBAL_ENABLED.get()
                || !ModConfig.HUD_SCOREBOARD_ENABLED.get()) {
            return component;
        }
        ...
        ITextComponent result = SafeTranslate.guard(() -> translateDirect(component,
    ...
    public static ITextComponent translateBelowNameComponent(ITextComponent component) {
    ...
    private static ITextComponent translateDirect(ITextComponent component, String surface, String role) {
        if (component == null || !ModConfig.GLOBAL_ENABLED.get()
                || !ModConfig.HUD_SCOREBOARD_ENABLED.get()
    ...
    public static String translateString(String text) {
        return SafeTranslate.guard(() -> {
            ...
            ITextComponent request = com.yourname.simpletranslate.core.LegacyComponentFactory.literal(text);
    ...
    private static final Map<ITextComponent, String> COMPONENT_KEY_MEMO = new IdentityHashMap<>();
    ...
    private static final class FrameCapture {
        ...
        private final List<ITextComponent> sources = new ArrayList<>();
        ...
        private ITextComponent collect(ITextComponent component) {
            ...
            ITextComponent translated = previous.translated.get(index);
    ...
    private record FrameSnapshot(List<String> sourceKeys, List<ITextComponent> translated) {
        private static FrameSnapshot of(List<String> sourceKeys, List<ITextComponent> translated) {
            return new FrameSnapshot(List.copyOf(sourceKeys), List.copyOf(translated));
        }
    }

    /**
     * Projects the complete ordered sidebar into one top-level ITextComponent array.
     *
    ...
    private static final class FrameProjection {
        private final List<ITextComponent> sources;
        private final List<Integer> rowIndexes;
        private final List<ITextComponent> requests;
        private final String context;

        private FrameProjection(List<ITextComponent> sources, List<Integer> rowIndexes,
                                List<ITextComponent> requests, String context) {
        ...
        private static FrameProjection project(List<ITextComponent> sources) {
            if (sources == null || sources.isEmpty()) {
                return null;
            }
            List<Integer> indexes = new ArrayList<>();
            List<ITextComponent> requests = new ArrayList<>();
            for (int index = 0; index < sources.size(); index++) {
                ITextComponent component = sources.get(index);
                ...
                indexes.add(index);
                // Keep the exact source ITextComponent tree. The shared visual
            ...
            for (ITextComponent source : sources) {
        ...
        private List<ITextComponent> requests() {
            return requests;
        }
        ...
        private List<ITextComponent> bind(List<ITextComponent> translatedRows) {
            if (translatedRows == null || translatedRows.size() != rowIndexes.size()) {
                return null;
            }
            List<ITextComponent> mapped = new ArrayList<>(sources);
            for (int ordinal = 0; ordinal < rowIndexes.size(); ordinal++) {
                int sourceIndex = rowIndexes.get(ordinal);
                ITextComponent translated = translatedRows.get(ordinal);
                if (translated == null) {
                    return null;
                }
