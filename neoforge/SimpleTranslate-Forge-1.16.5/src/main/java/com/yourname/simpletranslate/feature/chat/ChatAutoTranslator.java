...
public final class ChatAutoTranslator {
    ...
    private record CollectEntry(ITextComponent message, String plainText,
    ...
    void handleIncomingMessage(ITextComponent message, String plainText, TranslationManager manager) {
    ...
    private boolean tryApplyCachedTranslation(ITextComponent message, String plainText) {
    ...
    boolean tryApplyCachedAutoMessage(ChatMessageIdentity identity,
        ...
        ITextComponent translatedComponent = ChatTranslationRuntime.buildCachedAutoMessageTranslation(message);
    ...
    private void handleAutoMode(ITextComponent message, String plainText) {
        if (!ModConfig.GLOBAL_ENABLED.get()
    ...
    private void translateAutoDirect(ITextComponent message,
                                     String plainText,
    ...
    private void translateAutoDirectWithContext(ITextComponent message,
                                                String plainText,
                                                ChatMessageIdentity identity,
                                                String pendingKey) {
     ...
     * as one ordered ITextComponent JSON array.
     */
    void tick() {
        if (collectBuffer.isEmpty()) {
            return;
    ...
    private void flushCollectedBatch(List<CollectEntry> batch) {
        ...
        List<ITextComponent> messages = new ArrayList<>(batch.size());
    ...
    private static int componentJsonLength(ITextComponent component) {
    ...
    private void applyAutoTranslationIfCurrent(ChatMessageIdentity identity,
                                               ITextComponent translatedComponent,
    ...
    private boolean canApplyAutoTranslation(ChatMessageIdentity identity, ITextComponent translatedComponent) {
