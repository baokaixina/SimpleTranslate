...
import com.yourname.simpletranslate.gui.LegacyMultiLineEditBox;
...
/**
 * Edits one cached ITextComponent JSON translation as readable text while preserving
...
public class CacheEditScreen extends BaseSimpleTranslateScreen {
    ...
    private LegacyMultiLineEditBox translationInput;
    ...
    public CacheEditScreen(Screen parent, String cacheKey) {
        super(com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        this.translationInput = new LegacyMultiLineEditBox(this.font, left, inputY, panelWidth, inputHeight,
        ...
        this.saveButton = ButtonCompat.builder(
                com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.save"),
        ...
        if (entry == null) {
            status = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.error.missing-entry");
        }
        updateSaveButtonState();
    }
    ...
    private void saveEditedTranslation() {
        ...
        if (error.isPresent()) {
            status = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.error." + error.get());
            return;
        }
        ...
        status = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.saved");
        updateSaveButtonState();
    }
    ...
    public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawString(this.font, com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.translation"),
    ...
    private void drawSourcePanel(GuiGraphics graphics, int left, int top, int width, int height) {
        graphics.drawString(this.font, com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.source"),
        ...
        if (source == null || source.isBlank()) {
            source = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.source_missing").getString();
        }
        List<IReorderingProcessor> lines = this.font.split(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(source), width - 8);
        int y = top + 5;
    ...
    private void drawTranslationPreview(GuiGraphics graphics, int left, int top, int width, int height) {
        graphics.drawString(this.font, com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.cache.edit.preview"),
                left, top - 12, 0xFFFFFFFF);
        graphics.fill(left - 1, top - 1, left + width + 1, top + height + 1, 0xAA000000);
        ...
        String projectedJson = nodes.size() == editableNodeCount && entry != null
                ...
                : null;
        List<ITextComponent> projected = ComponentJsonCacheEditor.components(projectedJson);
        List<IReorderingProcessor> visualLines = new ArrayList<>();
        int wrapWidth = Math.max(20, width - 14);
        for (ITextComponent styled : projected) {
            List<IReorderingProcessor> wrapped = this.font.split(styled, wrapWidth);
            if (wrapped.isEmpty()) {
                visualLines.add(com.yourname.simpletranslate.core.LegacyComponentFactory.empty().getVisualOrderText());
            } else {
