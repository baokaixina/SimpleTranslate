...
public final class ChatTranslationController {
    ...
    private static final Set<ITextComponent> PROCESSED_MESSAGES =
            Collections.synchronizedSet(Collections.newSetFromMap(new IdentityHashMap<>()));
    private static final Map<UUID, ButtonMessageData> BUTTON_MESSAGES = new ConcurrentHashMap<>();
    private static final Map<ITextComponent, ITextComponent> AUTO_PEER_MAP =
    ...
    public Set<ITextComponent> processedMessages() {
    ...
    public Map<ITextComponent, ITextComponent> autoPeerMap() {
        return AUTO_PEER_MAP;
    }

    public void markProcessed(ITextComponent component) {
        if (component != null) {
        ...
        future.whenComplete((translated, error) -> {
            try {
                minecraft.execute(() -> {
                    ...
                    lastFailedOutgoingMessage = "";
                    screen.sendMessage(normalizedTranslation);
    ...
    public void onAddMessage(ITextComponent message) {
    ...
    public List<String> collectContextLines(ITextComponent message, String plainText, int[] targetIndexHolder) {
        ...
        for (int i = 0; i < allMessages.size(); i++) {
            ...
            if (hudHistoryPresenter.isHudHistoryChatMessage(msg.getMessage())) {
                continue;
            }
        ...
        for (int i = index - 1; i >= 0 && priorLines.size() < contextLimit; i--) {
            ITextComponent content = allMessages.get(i).getMessage();
            if (hudHistoryPresenter.isHudHistoryChatMessage(content)) {
                continue;
            }
            String text = getOriginalContextText(content);
    ...
    public String getOriginalContextText(ITextComponent content) {
        if (content == null) {
            return "";
        }
        ITextComponent originalPeer = AUTO_PEER_MAP.get(content);
        ...
        try {
            ...
            for (int i = 0; i < allMessages.size(); i++) {
                ...
                if (blocked) {
                    ...
                    ITextComponent originalWithButton = buttonController.createMessageWithButton(
    ...
    private static void showOutgoingStatus(Minecraft minecraft, String key, Object... args) {
        if (minecraft != null && minecraft.player != null) {
            minecraft.player.displayClientMessage(com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(key, args), true);
        }
    }
}
