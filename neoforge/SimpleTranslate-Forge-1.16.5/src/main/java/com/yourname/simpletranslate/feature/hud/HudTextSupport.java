...
public final class HudTextSupport {
    ...
    public static ActionbarTemplate actionbarTemplate(@Nullable ITextComponent original) {
        if (original == null) {
            return new ActionbarTemplate(com.yourname.simpletranslate.core.LegacyComponentFactory.empty(), List.of(), List.of());
        }
        List<TextSegmentInfo> segments = new ArrayList<>();
        ComponentSegmentHelper.extractSegments(original, segments, Style.EMPTY, false);
        IFormattableTextComponent normalized = com.yourname.simpletranslate.core.LegacyComponentFactory.empty();
    ...
    @Nullable
    public static ITextComponent restoreActionbarVariables(ITextComponent translatedTemplate, ActionbarTemplate template) {
        ...
        IFormattableTextComponent restored = com.yourname.simpletranslate.core.LegacyComponentFactory.empty();
        ...
        for (TextSegmentInfo segment : segments) {
            ...
            while (matcher.find()) {
                if (matcher.start() > cursor) {
                    restored.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(segment.text.substring(cursor, matcher.start())).withStyle(style));
                }
                ActionbarVariable variable = variableAt(variables, matcher.group(1));
                if (variable == null) {
                    restored.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(matcher.group()).withStyle(style));
                } else {
                    restored.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(variable.value()).withStyle(variable.style()));
                    restoredVariables++;
                }
                cursor = matcher.end();
            }
            if (cursor < segment.text.length()) {
                restored.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(segment.text.substring(cursor)).withStyle(style));
            }
        }
        return restoredVariables == variables.size() ? restored : null;
    }

    public static String componentStyleSignature(@Nullable ITextComponent component) {
        if (component == null) {
            return "";
        }
        List<TextSegmentInfo> segments = new ArrayList<>();
    ...
    private static void appendTemplateText(IFormattableTextComponent target, String text, Style style,
        ...
        while (cursor < text.length()) {
            ...
            target.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(literal).withStyle(effectiveStyle));
    ...
    private static void appendProtectedToken(IFormattableTextComponent target, String value, Style style,
            List<ActionbarVariable> variables, List<ActionbarLeaf> leaves) {
        if (value == null || value.isEmpty()) {
            return;
        }
        if (variables.size() >= MAX_ACTIONBAR_PLACEHOLDERS) {
            target.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(value).withStyle(style));
        ...
        target.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal(placeholder).withStyle(style));
    ...
    public record ActionbarTemplate(ITextComponent component, List<ActionbarVariable> variables,
        ...
        public ActionbarTemplate(ITextComponent component, List<ActionbarVariable> variables) {
