...
public final class ComponentSegmentHelper {
    ...
    public static String getDirectText(ITextComponent component) {
        ...
        if (component instanceof StringTextComponent literal) {
            return literal.getText() == null ? "" : literal.getText();
        }

        ITextComponent safeComponent = ComponentRenderSafety.sanitize(component);
        String full = safeComponent.getString();
        if (full.isEmpty()) {
            return full;
        }

        if (safeComponent.getSiblings().isEmpty()) {
            return full;
        }

        StringBuilder siblingsText = new StringBuilder();
        for (ITextComponent sibling : safeComponent.getSiblings()) {
            siblingsText.append(ComponentRenderSafety.sanitize(sibling).getString());
        }
    ...
    public static void extractSegments(ITextComponent component, List<TextSegmentInfo> segments, Style parentStyle,
            boolean includeEmptyLeaf) {
        extractSegments(component, segments, parentStyle, includeEmptyLeaf, 0);
    }

    private static void extractSegments(ITextComponent component, List<TextSegmentInfo> segments, Style parentStyle,
            boolean includeEmptyLeaf, int depth) {
        ...
        if (safeComponent != component) {
            extractSegments(safeComponent, segments, parentStyle, includeEmptyLeaf, depth + 1);
            return;
        }
        ...
        if (component instanceof TranslationTextComponent translatable) {
            ...
            collectTranslatableArgSegments(translatable, visitStyle, argSegments, includeEmptyLeaf, depth);

            StringBuilder visitedText = new StringBuilder();
            List<TextSegmentInfo> visitedSegments = new ArrayList<>();
            translatable.visit((style, text) -> {
            ...
            if (!argSegments.isEmpty() && concatSegmentTexts(argSegments).equals(resolved)) {
                ...
                supplementTranslatableArgs(translatable, component, visitStyle, segments, resolved, includeEmptyLeaf, depth);
            }
            ...
            for (ITextComponent sibling : component.getSiblings()) {
                extractSegments(sibling, segments, visitStyle, includeEmptyLeaf, depth + 1);
            }
            return;
        }
        ...
        for (ITextComponent sibling : component.getSiblings()) {
            extractSegments(sibling, segments, mergedStyle, includeEmptyLeaf, depth + 1);
        }
    }

    private static void collectTranslatableArgSegments(TranslationTextComponent translatable, Style parentStyle,
                                                      List<TextSegmentInfo> segments, boolean includeEmptyLeaf, int depth) {
        ...
        for (Object arg : args) {
            if (arg instanceof ITextComponent argComponent) {
                extractSegments(argComponent, segments, parentStyle, includeEmptyLeaf, depth + 1);
            }
        }
    }
     ...
     */
    private static void supplementTranslatableArgs(TranslationTextComponent translatable, ITextComponent component,
                                                   Style parentStyle, List<TextSegmentInfo> segments,
                                                   String visitedText, boolean includeEmptyLeaf, int depth) {
        if (translatable == null || segments == null) {
            return;
        }
        if (depth > MAX_SEGMENT_DEPTH) {
        ...
        for (Object arg : args) {
            if (arg instanceof ITextComponent argComponent) {
                ITextComponent safeArg = ComponentRenderSafety.sanitize(argComponent);
                String argText = safeArg.getString();
                if (argText == null || argText.isEmpty()) {
                    continue;
                }
                if (resolved.contains(argText)) {
                    if (!hasStyledArgRepresentation(segments, safeArg)) {
                        removeUnstyledArgText(segments, argText);
                        extractSegments(safeArg, segments, parentStyle, includeEmptyLeaf, depth + 1);
                    }
                    continue;
                }
                extractSegments(safeArg, segments, parentStyle, includeEmptyLeaf, depth + 1);
    ...
    private static boolean hasStyledArgRepresentation(List<TextSegmentInfo> segments, ITextComponent argComponent) {
        if (segments == null || segments.isEmpty() || argComponent == null) {
            return false;
        }
        List<TextSegmentInfo> argSegments = new ArrayList<>();
        extractSegments(argComponent, argSegments, Style.EMPTY, false, 0);
