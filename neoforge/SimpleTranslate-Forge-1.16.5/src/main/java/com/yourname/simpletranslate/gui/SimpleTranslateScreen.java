...
public class SimpleTranslateScreen extends BaseSimpleTranslateScreen {
    ...
    private final List<Widget> scrollableWidgets = new ArrayList<>();
    ...
    public SimpleTranslateScreen(Screen parent) {
        super(com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.settings"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        if (isExpanded(Section.GENERAL)) {
            CycleButton<Boolean> globalButton = CycleButton.onOffBuilder(ModConfig.GLOBAL_ENABLED.get())
                    .create(this.width / 2 - buttonWidth / 2, y, buttonWidth, BUTTON_HEIGHT,
                            com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.settings.global_enabled"),
                            (button, enabled) -> {
        ...
        if (isExpanded(Section.SCOPE)) {
            ...
            y = addPageButton(y, buttonWidth, "screen.simple_translate.main.item_tooltip",
                    ...
                    shortcutHint(net.minecraft.util.text.TextFormatting.GRAY,
        ...
        this.backButton = new HoverHighlightButton(this.width / 2 - buttonWidth / 2, this.height - 25,
                buttonWidth, BUTTON_HEIGHT, com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.back"),
                button -> onClose());
    ...
    private int addPageButton(int y, int width, String labelKey, String tooltipKey,
                              ScreenFactory factory, ITextComponent shortcutSuffix) {
        Button button = new HoverHighlightButton(this.width / 2 - width / 2, y, width, BUTTON_HEIGHT,
                com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(labelKey), ignored -> Minecraft.getInstance().setScreen(factory.create()));
        if (shortcutSuffix == null) {
            withTooltip(button, tooltipKey);
        } else {
            withTooltip(button, com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(tooltipKey)
                    .append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal("\n")).append(shortcutSuffix));
        }
        addScrollable(button, y);
        return y + SPACING;
    }

    private static ITextComponent shortcutHint(net.minecraft.util.text.TextFormatting formatting,
                                          com.yourname.simpletranslate.keybind.ShortcutAction... actions) {
        IFormattableTextComponent hint = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.shortcuts.hint_prefix")
                .withStyle(formatting);
        for (int index = 0; index < actions.length; index++) {
            if (index > 0) {
                hint.append(com.yourname.simpletranslate.core.LegacyComponentFactory.literal("/").withStyle(formatting));
            }
    ...
    private void addScrollable(Widget widget, int baseY) {
    ...
    private void repositionWidgets() {
        ...
        for (int i = 0; i < this.scrollableWidgets.size(); i++) {
            Widget widget = this.scrollableWidgets.get(i);
    ...
    public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawCenteredString(this.font,
                com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.main.subtitle"),
        ...
        ITextComponent statusText = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(ready
        ...
        if (recentError != null) {
            ITextComponent error = com.yourname.simpletranslate.core.LegacyComponentFactory.translatable("screen.simple_translate.status.api_error");
        ...
        for (SectionLabel label : this.sectionLabels) {
            ...
            if (y >= VIEWPORT_TOP - 10 && y < contentBottom()) {
                graphics.drawCenteredString(this.font, com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(label.key),
    ...
    private final class SectionHeaderWidget extends Widget {
        private final Section section;

        private SectionHeaderWidget(Section section, int width) {
            super(SimpleTranslateScreen.this.width / 2 - width / 2, 0, width, 18,
                    com.yourname.simpletranslate.core.LegacyComponentFactory.translatable(section.labelKey));
        ...
        public void renderButton(MatrixStack poseStack, int mouseX, int mouseY, float partialTick) {
            ...
            if ((isHovered() || isFocused())) {
                graphics.fill(x, y, x + this.width, y + this.height, 0x33111111);
            }
            ITextComponent arrow = com.yourname.simpletranslate.core.LegacyComponentFactory.literal(isExpanded(this.section) ? "\u25be " : "\u25b8 ");
            ITextComponent line = arrow.copy().append(getMessage());
            graphics.drawString(SimpleTranslateScreen.this.font, line,
                    x + 4, y + 5, (isHovered() || isFocused()) ? 0xFFFFFFFF : 0xFFE0E0E0, false);
        }
        ...
        public void updateNarration(NarrationElementOutput output) {
            // 1.16.5 has no public default widget narration helper.
        }
    }
