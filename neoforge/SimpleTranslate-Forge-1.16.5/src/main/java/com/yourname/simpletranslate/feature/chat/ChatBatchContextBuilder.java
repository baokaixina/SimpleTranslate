...
/** Builds prior visible-chat context for ITextComponent JSON chat requests. */
final class ChatBatchContextBuilder {
    ...
    static String buildDirectContext(ChatTranslationController controller,
        ...
        for (int i = allMessages.size() - 1; i >= 0 && merged.size() < contextLimit; i--) {
            ChatLine<ITextComponent> guiMessage = allMessages.get(i);
            if (guiMessage == null || guiMessage.getMessage() == null) {
                continue;
            }
            ITextComponent content = guiMessage.getMessage();
    ...
    static String resolveTranslatedText(ChatTranslationController controller,
                                        ITextComponent content,
                                        @Nullable ITextComponent translatedComponent) {
