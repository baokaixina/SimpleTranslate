...
public final class TooltipSemanticResultStore {
    ...
    private static final Map<String, List<ITextComponent>> READY =
            Collections.synchronizedMap(new LinkedHashMap<>(128, 0.75F, true) {
                private static final long serialVersionUID = 1L;

                @Override
                protected boolean removeEldestEntry(Map.Entry<String, List<ITextComponent>> eldest) {
    ...
    @Nullable
    public static List<ITextComponent> get(String key) {
        if (key == null || key.isBlank()) {
            return null;
        }
        List<ITextComponent> value = READY.get(key);
        return value == null ? null : List.copyOf(value);
    }

    public static void put(String key, List<ITextComponent> translatedSemantic) {
