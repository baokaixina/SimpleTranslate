...
Write-Host "SimpleTranslate 1.21 logic checks (product sync from 1.21.3 donor, Wynn stripped for <1.21.4)"
...
Assert-PathMissing "$src/feature/wynn" "Wynn package must not ship on Minecraft 1.21  (< 1.21.4)"
...
Assert-PathMissing "$src/mixin/compat/WynntilsOverlayManagerMixin.java" "Wynntils compat mixin must not ship on 1.21"
...
Assert-FileNotContains "$src/config/ModConfig.java" "HUD_WYNN_OVERLAY_ENABLED" "Wynn overlay switch must not ship on 1.21"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "Wynn" "GUI helper must carry no Wynn overlay layer on 1.21"
Assert-FileNotContains "$src/feature/hud/HudFeature.java" "Wynn" "HudFeature must carry no Wynn layer on 1.21"
Assert-FileNotContains "$res/assets/simple_translate/lang/zh_cn.json" "hud.wynn" "zh Wynn HUD keys must not ship on 1.21"
Assert-FileNotContains "$res/assets/simple_translate/lang/en_us.json" "hud.wynn" "en Wynn HUD keys must not ship on 1.21"
...
# NeoForge 21.0 ships Mixin 0.8.6 without @Inject.order; the HUD frame bracket
# is a default-order HEAD/RETURN @Inject pair on Gui.render instead.
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'at = @At("HEAD"))' "HUD frame must open at Gui.render HEAD (21.0: default order, Mixin 0.8.6 has no @Inject.order)"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'at = @At("RETURN"))' "HUD frame must close at Gui.render RETURN (21.0: default order, Mixin 0.8.6 has no @Inject.order)"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "DedicatedHudSurface" "known dedicated HUD surfaces must be permanently isolated from K"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" '"renderOverlayMessage(Lnet/minecraft/client/gui/GuiGraphics;Lnet/minecraft/client/DeltaTracker;)V"' "actionbar isolation must use the exact vanilla render owner"
# NeoForge 21.0 has no WrapMethod (MixinExtras 0.3.5), so the dedicated HUD
# suppression bracket is a HEAD/RETURN pair; there is no finally-style
# exception-path closure on this target (Gui.render throwing crashes anyway).
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "endCaptureSuppression" "dedicated HUD ownership scope must close after the surface renders"
...
Write-Host "1.21 logic checks PASSED"
