...
Write-Host "SimpleTranslate 1.20.2 logic checks (product sync from 1.20.2 Fabric donor)"
...
Assert-PathMissing "$src/feature/wynn" "Wynn package must not ship on Minecraft 1.20.2"
...
Assert-PathMissing "$src/mixin/compat/WynntilsOverlayManagerMixin.java" "Wynntils compat mixin must not ship on 1.20.2"
...
Assert-FileNotContains "$src/config/ModConfig.java" "HUD_WYNN_OVERLAY_ENABLED" "Wynn overlay switch must not ship on 1.20.2"
...
Assert-FileNotContains "$res/assets/simple_translate/lang/zh_cn.json" "hud.wynn" "zh Wynn HUD keys must not ship on 1.20.2"
Assert-FileNotContains "$res/assets/simple_translate/lang/en_us.json" "hud.wynn" "en Wynn HUD keys must not ship on 1.20.2"
...
Assert-FileNotContains "$src/feature/hud/HudFeature.java" "Wynn" "HudFeature must carry no Wynn layer on 1.20.2"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "Wynn" "GUI helper must carry no Wynn overlay layer on 1.20.2"
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "DialogScreen" "1.20.2 has no dialog screen API"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "hud.wynn" "no Wynn overlay cache surface may ship on 1.20.2"
...
Assert-FileNotContains "$src/core/JsonPassthroughPipeline.java" ".wynn." "pipeline must carry no Wynn recovery branches on 1.20.2"
Assert-FileContains "$src/config/ModConfig.java" "CONTENT_HUD_FRAME_ACTIVE" "HUD K activation persistence required"
Assert-FileContains "$src/SimpleTranslateClientBootstrap.java" "migrateLegacyHudFrameActivation" "existing HUD K cache must migrate to persistent activation (client bootstrap after NeoForge physical-side split)"
...
Assert-FileNotContains "$src/transport/JsonPassthroughPrompts.java" "Wynncraft" "prompts must carry no Wynn branches on 1.20.2"
...
Assert-FileContains "$src/mixin/ScreenGuiTranslationMixin.java" "renderWithTooltip(" "1.20.2 GUI frame scope uses renderWithTooltip"
Assert-FileContains "$src/mixin/ScoreboardMixin.java" 'method = "displayScoreboardSidebar(Lnet/minecraft/client/gui/GuiGraphics;Lnet/minecraft/world/scores/Objective;)V"' "1.20.2 draws sidebar text inline; no drawManaged lambda exists"
Assert-FileNotContains "$src/mixin/ScoreboardMixin.java" 'method = "method_55440"' "intermediary target names never resolve on the NeoForge mojmap runtime"
Assert-FileNotContains "$src/mixin/ScoreboardMixin.java" "lambda`$displayScoreboardSidebar" "the drawManaged sidebar lambda only exists on 1.20.3+"
...
Assert-FileContains "$src/mixin/HoverTooltipMixin.java" "renderTooltipInternal" "1.20.2 tooltips funnel through renderTooltipInternal"
...
Assert-FileNotContains "$src/mixin/TitleOverlayMixin.java" "Wynn" "title overlay must carry no Wynn hooks on 1.20.2"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "endHudFrame" "HUD frame end"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'Mixin 0.8.5 (NeoForge 20.2.x) has no Inject#order' "documented NeoForge injector-order adaptation must stay"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'at = @At("HEAD")' "HUD frame must open at Gui.render HEAD"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'at = @At("RETURN")' "HUD frame must close at Gui.render RETURN"
...
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" '"render(Lnet/minecraft/client/gui/GuiGraphics;F)V"' "1.20.2 HUD frame must hang on the exact Gui.render owner"
...
Assert-FileNotContains "$src/mixin/TextDisplayMixin.java" "wynn" "text display must carry no Wynn label surface on 1.20.2"
...
Assert-FileContains "$src/keybind/ModKeyBindings.java" '"key.category.simple_translate.general"' "1.20.2 KeyMapping category is a plain translation-key string"
...
Assert-FileNotContains "$src/cache/SharedCachePayload.java" "implements CustomPacketPayload" "1.20.2 shared cache stays a plain SimpleChannel message (no payload stack on 20.2)"
Assert-FileContains "$src/cache/SharedCachePayload.java" 'new ResourceLocation("simple_translate", "cache_sync/v1")' "1.20.2 shared cache channel id"
Assert-FileContains "$src/cache/SharedCacheNetworking.java" "NetworkRegistry.newSimpleChannel" "1.20.2 shared cache registration uses the 20.2 SimpleChannel API"
...
# NeoForge 1.20.2: no Iceberg artifact exists for Minecraft 1.20.2 on any NeoForge
# build (Modrinth project 5faXoLqX lists none; the 1.20.1 Forge build requires mod id
# "forge", which NeoForge 20.2 does not provide). FTB Library likewise has no loadable
# artifact: maven.ftb.dev publishes no ftb-library-neoforge 2002.x/2003.x series, and
# the ftb-library-forge-2001.x builds require mod id "forge". Both optional compat
# surfaces are blocked by the evidence gate; assert the omissions are documented.
Assert-PathMissing "$src/compat/IcebergTooltipGatherCompat.java" "blocked Iceberg gather compat must not ship on 1.20.2"
Assert-FileContains "$src/SimpleTranslateClientBootstrap.java" "Iceberg" "blocked Iceberg gather compat must document the missing artifact"
Assert-PathMissing "$src/mixin/compat/FtbScreenWrapperTranslationMixin.java" "blocked FTB screen bridge must not ship on 1.20.2"
Assert-PathMissing "$src/mixin/compat/FtbTextFieldTranslationMixin.java" "blocked FTB text-field bridge must not ship on 1.20.2"
Assert-PathMissing "$src/mixin/SimpleTranslateMixinPlugin.java" "with no optional mixins left the gating plugin must be deleted, not emptied"
Assert-FileNotContains "$res/simple_translate.mixins.json" "Ftb" "blocked FTB mixins must not be registered"
Assert-FileNotContains "$res/simple_translate.mixins.json" '"plugin"' "retired mixin plugin must not be referenced"
