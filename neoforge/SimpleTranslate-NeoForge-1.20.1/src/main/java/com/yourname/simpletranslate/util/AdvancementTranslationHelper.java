 ...
 *
 * Keep advancement entry points on the direct document pipeline. Older
 * marker/raw caches are intentionally bypassed here because advancement titles,
...
public final class AdvancementTranslationHelper {
    ...
    public static void ensureTranslation(String advancementId, Component title, Component description) {
        ...
        String descText = description == null ? "" : description.getString();
                    ...
                    try {
                        ...
                        if (result != null && result.translated && result.components != null && result.components.size() == 2) {
                            ...
                            SimpleTranslateMod.getLogger().debug(
                                    ...
                                    documentKey, safeSummary(result.components.get(0).getString()),
                                    safeSummary(result.components.get(1).getString()));
                        }
                    } finally {
    ...
    public static Component getCachedTitleComponent(Component original) {
        return getCachedDocumentComponent(original, TITLE_SURFACE);
    }
    ...
    public static Component getCachedDescriptionComponent(Component title, Component description) {
        ...
        DirectFormattedTranslationPipeline.ComponentListResult document =
        ...
        if (document.handled && document.translated && document.components != null && document.components.size() == 2) {
            ...
            if (translatedTitle != null && title != null) {
    ...
    public static Component getCachedDescriptionComponent(Component original) {
        return getCachedDocumentComponent(original, DESCRIPTION_SURFACE);
    }
    ...
    private static Component rebuildFromDocumentCache(Component original, String text, String surface) {
        ...
        if (translated == null || isRejected(text, translated)) {
            ...
            if (cache != null && ModConfig.CACHE_ENABLED.get()) {
                translated = cache.get(TranslationCacheKeys.key(surface, text)).orElse(null);
                if (translated != null && !isRejected(text, translated)) {
                    stringCache.put(componentKey(surface, text), translated);
                }
            }
        }
        if (translated == null || isRejected(text, translated)) {
        ...
        Component rebuilt = rebuildComponentWithTranslation(original, translated);
        if (rebuilt == null) {
            return null;
        }
        return rebuilt;
    ...
    public static String getCachedTranslation(String text) {
        ...
        TranslationCache cache = SimpleTranslateMod.getTranslationCache();
        if (cache != null && ModConfig.CACHE_ENABLED.get()) {
            var persistent = cache.get(TranslationCacheKeys.key(TITLE_SURFACE, text));
            if (persistent.isPresent() && !isRejected(text, persistent.get())) {
                stringCache.put(componentKey(TITLE_SURFACE, text), persistent.get());
    ...
    public static Component rebuildComponentWithTranslation(Component original, String translatedText) {
        if (original == null) {
            return Component.literal(translatedText == null ? "" : translatedText);
        }
        String originalText = original.getString();
        if (translatedText == null || translatedText.isEmpty() || translatedText.equals(originalText)) {
            return original;
        }
        List<TextSegmentInfo> segments = new java.util.ArrayList<>();
        ComponentSegmentHelper.extractSegments(original, segments, Style.EMPTY, true);
        if (segments.size() > 1) {
            SimpleTranslateMod.getLogger().debug(
                    "Advancement component kept original because translated text has no slot-level style map: '{}'",
                    originalText);
            return original;
        }
        Style style = original.getStyle();
        return Component.literal(translatedText).withStyle(style == null ? Style.EMPTY : style);
    }
