...
public final class ActiveFontManager {
    ...
    /**
     * Bumps the resource revision without replacing the captured manager.
     * Forge adaptation: the 1.19.x font reload runs inside the anonymous
     * {@code FontManager$1}; its synthetic outer field cannot be shadowed on
     * ForgeGradle 0.7.38 / Mixin 0.8.5 (the annotation processor crashes
     * printing the {@code this$0} target), so the reload hook only bumps the
     * revision while the manager itself is captured once in the
     * {@code FontManager} constructor injector.
     */
