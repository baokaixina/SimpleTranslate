 ...
 * {@code Iceberg-1.20.1-forge-1.1.25.jar} (the newest Forge build declared for
 * 1.20.1 on Modrinth, sha1 d551ff55814cfb01403b812459ee5a02b020d87b): the
 * Forge Iceberg build has no Fabric-style {@code RenderTooltipEvents.GATHER};
 * instead {@code GatherComponentsExtEvent} extends Forge's
 * {@link RenderTooltipEvent.GatherComponents} (constructor
 * (ItemStack, int, int, List&lt;Either&lt;FormattedText, TooltipComponent&gt;&gt;, int, int),
 * adding only {@code getIndex()} — javap -p -s verified). The Forge base class
 * exposes {@code getTooltipElements()}, {@code getItemStack()},
 * {@code getScreenWidth()}, {@code getScreenHeight()} and {@code getMaxWidth()}.
 * The listener is registered by reflection against the ext class so only the
 * Iceberg/LT pipeline is affected; the handler then consumes it through the
 * compile-time Forge base type. The actual translation runs
 * through {@link TooltipTranslationHelper#translateGatheredTooltipLines},
