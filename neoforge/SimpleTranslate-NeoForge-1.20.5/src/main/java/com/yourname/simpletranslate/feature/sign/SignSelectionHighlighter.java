...
/**
 * NeoForge 20.5 port: RenderLevelStageEvent has no per-stage subclasses on
 * 20.5 (verified against the neoforge-20.5.21-beta universal jar), so the handler
 * filters on Stage.AFTER_ENTITIES, and the line box uses LevelRenderer
 * (ShapeRenderer only took over renderLineBox on later Minecraft versions).
 */
public final class SignSelectionHighlighter {
    ...
    private static void simple_translate$renderSelectionsImpl(RenderLevelStageEvent event) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || event == null) {
            return;
        }
        ...
        for (SignContextSelectionManager.SelectionView selection :
            ...
            LevelRenderer.renderLineBox(event.getPoseStack(), lines, box,
