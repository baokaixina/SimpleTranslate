...
/**
 * NeoForge entrypoint. Common (server-safe) wiring lives here; every
 * client-only surface is delegated to {@link SimpleTranslateClientBootstrap},
 * which is only initialized when the physical side is a client, so dedicated
 * server startup never loads client-only classes.
 */
