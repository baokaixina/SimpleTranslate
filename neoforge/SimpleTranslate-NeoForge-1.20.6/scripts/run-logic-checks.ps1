...
Write-Host "SimpleTranslate 1.20.6 logic checks (product sync from 1.21.3 donor, Wynn stripped for <1.21.4)"
...
if ($javaCount -lt 150) { throw "expected >=150 java files after donor sync with Wynn strip, got $javaCount" }

Assert-PathMissing "$src/feature/wynn" "Wynn package must not ship on Minecraft 1.21.3 (< 1.21.4)"
...
Assert-PathMissing "$src/mixin/compat/WynntilsOverlayManagerMixin.java" "Wynntils compat mixin must not ship on 1.21.3"
...
Assert-FileNotContains "$src/config/ModConfig.java" "HUD_WYNN_OVERLAY_ENABLED" "Wynn overlay switch must not ship on 1.21.3"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "Wynn" "GUI helper must carry no Wynn overlay layer on 1.21.3"
Assert-FileNotContains "$src/feature/hud/HudFeature.java" "Wynn" "HudFeature must carry no Wynn layer on 1.21.3"
Assert-FileNotContains "$res/assets/simple_translate/lang/zh_cn.json" "hud.wynn" "zh Wynn HUD keys must not ship on 1.21.3"
Assert-FileNotContains "$res/assets/simple_translate/lang/en_us.json" "hud.wynn" "en Wynn HUD keys must not ship on 1.21.3"
...
# NeoForge loader adaptation: the client entrypoint lives in SimpleTranslateClientBootstrap.java.
Assert-FileContains "$src/SimpleTranslateClientBootstrap.java" "migrateLegacyHudFrameActivation" "existing HUD K cache must migrate to persistent activation"
...
Assert-FileContains "$src/mixin/ScreenGuiTranslationMixin.java" "renderWithTooltip" "GUI frame scope (1.21.8: renderWithTooltip + renderDeferredTooltip)"
...
# NeoForge 20.6 ships sponge-mixin 0.13.4+mixin.0.8.5 whose @Inject has no `order`
# element (javap-verified), so the 900/1100 ordering cannot be expressed on this
# target; the late-overlay interaction it guards is the >=1.21.4 Wynntils surface,
# which this Wynn-free target does not ship. Assert the frame brackets instead.
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'at = @At("HEAD"))' "HUD frame must open at render HEAD"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'at = @At("RETURN"))' "HUD frame must close at render RETURN"
...
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" '"renderOverlayMessage(Lnet/minecraft/client/gui/GuiGraphics;F)V"' "actionbar isolation must use the exact vanilla render owner"
...
# NeoForge 1.20.6: no Iceberg artifact exists for Minecraft 1.20.6 on any loader
# (Modrinth project 5faXoLqX lists no 1.20.5/1.20.6 builds), so the gather compat
# is blocked by the evidence gate; assert the omission is documented instead.
...
Write-Host "1.20.6 logic checks PASSED"
