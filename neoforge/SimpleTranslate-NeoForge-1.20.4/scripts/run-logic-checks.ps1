...
Write-Host "SimpleTranslate 1.20.4 logic checks (product sync from 1.20.4 Fabric donor)"
...
Assert-PathMissing "$src/feature/wynn" "Wynn package must not ship on Minecraft 1.20.4"
...
Assert-PathMissing "$src/mixin/compat/WynntilsOverlayManagerMixin.java" "Wynntils compat mixin must not ship on 1.20.4"
...
Assert-FileNotContains "$src/config/ModConfig.java" "HUD_WYNN_OVERLAY_ENABLED" "Wynn overlay switch must not ship on 1.20.4"
...
Assert-FileNotContains "$res/assets/simple_translate/lang/zh_cn.json" "hud.wynn" "zh Wynn HUD keys must not ship on 1.20.4"
Assert-FileNotContains "$res/assets/simple_translate/lang/en_us.json" "hud.wynn" "en Wynn HUD keys must not ship on 1.20.4"
...
Assert-FileNotContains "$src/feature/hud/HudFeature.java" "Wynn" "HudFeature must carry no Wynn layer on 1.20.4"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "Wynn" "GUI helper must carry no Wynn overlay layer on 1.20.4"
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "DialogScreen" "1.20.4 has no dialog screen API"
...
Assert-FileNotContains "$src/feature/gui/GuiTranslationHelper.java" "hud.wynn" "no Wynn overlay cache surface may ship on 1.20.4"
...
Assert-FileNotContains "$src/core/JsonPassthroughPipeline.java" ".wynn." "pipeline must carry no Wynn recovery branches on 1.20.4"
...
Assert-FileNotContains "$src/transport/JsonPassthroughPrompts.java" "Wynncraft" "prompts must carry no Wynn branches on 1.20.4"
...
Assert-FileContains "$src/mixin/ScreenGuiTranslationMixin.java" "renderWithTooltip(" "1.20.4 GUI frame scope uses renderWithTooltip"
Assert-FileContains "$src/mixin/ScoreboardMixin.java" "lambda`$displayScoreboardSidebar`$4" "1.20.4 sidebar lambda must use the mojmap synthetic name"
...
Assert-FileContains "$src/mixin/HoverTooltipMixin.java" "renderTooltipInternal" "1.20.4 tooltips funnel through renderTooltipInternal"
...
Assert-FileNotContains "$src/mixin/TitleOverlayMixin.java" "Wynn" "title overlay must carry no Wynn hooks on 1.20.4"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "endHudFrame" "HUD frame end"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'Mixin 0.8.5 (NeoForge 20.4.x) has no Inject#order' "documented NeoForge injector-order adaptation must stay"
...
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" '"render(Lnet/minecraft/client/gui/GuiGraphics;F)V"' "1.20.4 HUD frame must hang on the exact Gui.render owner"
...
Assert-FileNotContains "$src/mixin/TextDisplayMixin.java" "wynn" "text display must carry no Wynn label surface on 1.20.4"
...
Assert-FileContains "$src/keybind/ModKeyBindings.java" '"key.category.simple_translate.general"' "1.20.4 KeyMapping category is a plain translation-key string"
...
Assert-FileContains "$src/cache/SharedCachePayload.java" "implements CustomPacketPayload" "1.20.4 shared cache rides the payload API"
Assert-FileContains "$src/cache/SharedCachePayload.java" 'new ResourceLocation("simple_translate", "cache_sync/v1")' "1.20.4 shared cache channel id"
Assert-FileContains "$src/cache/SharedCacheNetworking.java" "RegisterPayloadHandlerEvent" "1.20.4 payload registration uses the registrar event"
...
# NeoForge 1.20.4: Iceberg has no NeoForge artifact for Minecraft 1.20.4 (the only
# 1.20.4 build on Modrinth project 5faXoLqX targets legacy Forge and requires mod id
# "forge" [49,), which NeoForge 20.4 does not provide), so the gather compat is blocked
# by the evidence gate; assert the omission is documented instead.
Assert-PathMissing "$src/compat/IcebergTooltipGatherCompat.java" "blocked Iceberg gather compat must not ship on 1.20.4"
...
Write-Host "1.20.4 logic checks PASSED"
