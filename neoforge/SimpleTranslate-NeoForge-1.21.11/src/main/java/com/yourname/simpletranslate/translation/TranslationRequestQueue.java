...
public final class TranslationRequestQueue {
    ...
    public static int cancelSurfacePrefix(String surfacePrefix) {
        ...
        if (canceled > 0) {
            SimpleTranslateMod.getLogger().debug(
                    "Translation queue canceled queued tasks surfacePrefix={} count={}",
    ...
    private static String normalizeKey(String key) {
        return key == null || key.isBlank() ? "anonymous:" + (++nextSequence) : key;
    }
    ...
    private static String laneForSurface(String surface) {
        String value = surface == null ? "" : surface.toLowerCase(Locale.ROOT);
        ...
        if (value.startsWith("hud.")) {
            return "hud";
        }
        if (value.startsWith("scoreboard")) {
            return "scoreboard";
        }
        if (value.startsWith("bossbar.")) {
            return "bossbar";
        }
        ...
        if (value.startsWith("entity.") || value.startsWith("text_display.")) {
