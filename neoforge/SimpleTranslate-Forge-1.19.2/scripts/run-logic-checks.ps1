...
Write-Host "SimpleTranslate Forge 1.19.2 logic checks (product sync from fabric 1.19.2 donor)"
...
# Forge 1.19.2 runtime replaces vanilla Gui.render with ForgeGui.render
# (verified against forge-1.19.2-43.5.2 bytecode, 2026-07-26/27): Gui.render
# injections/wraps never fire. The HUD frame window and field swap must hang
# on the RenderGuiEvent.Pre/Post pair that the live ForgeGui.render posts, and
# dedicated-surface capture suppression on RenderGuiOverlayEvent around the
# six vanilla overlays that own their own translation surfaces.
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "RenderGuiEvent.Pre.class" "HUD frame must open on the live ForgeGui RenderGuiEvent.Pre"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "RenderGuiEvent.Post.class" "HUD frame must close on the live ForgeGui RenderGuiEvent.Post"
Assert-FileNotContains "$src/mixin/TitleOverlayMixin.java" '"render(Lcom/mojang/blaze3d/vertex/PoseStack;F)V"' "dead Gui.render hooks must not return (ForgeGui.render never calls them)"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "RenderGuiOverlayEvent.Pre.class" "dedicated overlays must enter capture suppression"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "RenderGuiOverlayEvent.Post.class" "dedicated overlays must leave capture suppression"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "VanillaGuiOverlay.BOSS_EVENT_PROGRESS.id()" "boss bar overlay stays outside K ownership"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "VanillaGuiOverlay.CHAT_PANEL.id()" "chat overlay stays outside K ownership"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "VanillaGuiOverlay.SCOREBOARD.id()" "scoreboard overlay stays outside K ownership"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "VanillaGuiOverlay.PLAYER_LIST.id()" "tab list overlay stays outside K ownership"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "VanillaGuiOverlay.TITLE_TEXT.id()" "title overlay stays outside K ownership"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "VanillaGuiOverlay.RECORD_OVERLAY.id()" "actionbar overlay stays outside K ownership"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" "isActiveGui" "HUD listeners must ignore stale Gui instances"
Assert-FileContains "$src/mixin/TitleOverlayMixin.java" 'method = "<init>", at = @At("TAIL")' "HUD listeners must register once per Gui instance"
...
Assert-PathMissing "$src/mixin/TextDisplayMixin.java" "Display entity is 1.19.4+ only; text display mixin must not ship on 1.19.2"
Assert-FileNotContains "$res/simple_translate.mixins.json" "TextDisplayMixin" "text display mixin must not be registered on 1.19.2"
...
Assert-FileContains "$src/compat/IcebergTooltipGatherCompat.java" "GatherComponentsExtEvent" "Iceberg gather compat must hook the Forge GatherComponents ext event"
...
Write-Host "Forge 1.19.2 logic checks PASSED"
