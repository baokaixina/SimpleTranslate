...
function Get-SingleplayerWorldName {
    ...
    # Validation owns only its named smoke save. Never choose a user's most
    # recent singleplayer world merely because this dedicated client happens
    # to contain one; absence deliberately triggers isolated world creation.
    return $null
}
...
function Enable-PerMonitorDpiCoordinates {
    # Keep client-area measurements, client-to-screen conversion, cursor input
    # and screenshot capture in physical pixels. On a DPI-virtualized host the
    # old mixed coordinate spaces move a 1.19.2 Singleplayer click onto another
    # menu row, so an interactive smoke world can never be entered.
    try {
        [CodexUser32]::SetThreadDpiAwarenessContext([IntPtr](-4)) | Out-Null
    } catch {
        # Preserve the legacy behavior if this Windows build lacks PMv2.
    }
}
...
function Get-SingleplayerButtonYRatio {
    ...
    # Exact Forge 43.5.2 screenshot evidence: in the designated 1.19.2 test
    # profile the Singleplayer button is centred at half the physical client
    # height. The classic-layout estimate lands above the rendered button.
    if ($MinecraftVersion -eq '1.19.2') {
        return 0.50
    }
...
function Invoke-CreateSingleplayerWorldUi {
    ...
    # Exact 1.19.2 SelectWorldScreen places Create New World at (0.68, 0.83).
    # Other classic targets keep their existing direct-create flow.
    if ($MinecraftVersion -eq '1.19.2') {
        Invoke-WindowClick -Handle $handle -XRatio 0.68 -YRatio 0.83
        Start-Sleep -Seconds 2
    } else {
        # With no saves, 1.20.1 opens the Create World screen directly. If a
        # no-valid-worlds list is shown instead, the first bottom-left click
        # opens that screen and the second one creates the world.
...
function Invoke-EnterSingleplayerWorldUi {
    ...
    # World list: click the FIRST row, then press the bottom-left "play
    # selected world"
