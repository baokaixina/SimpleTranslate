 ...
 *
 * <p>API surface verified against the Iceberg-1.21.4-neoforge-1.2.13 jar (no
 * NeoForge Iceberg build targets Minecraft 1.21.6–1.21.8; the reflection-only
 * integration stays inert until a loadable Iceberg appears): the event class
 * lives at {@code com.anthonyhilyard.iceberg.events.client.RenderTooltipEvents}
 * and its {@code GATHER} is Iceberg's own public
 * {@code com.anthonyhilyard.iceberg.events.Event}, with
 * {@code Gather#onGather(ItemStack,int,int,List,int,int)} and a public
 * {@code GatherResult(InteractionResult,int,List)} record constructor — the
 * same shape as the Fabric 1.3.x series. The actual translation runs through
