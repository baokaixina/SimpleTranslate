...
if (-not (Test-Path -LiteralPath $realVersionJar)) {
    ...
    $candidateJarVersionIds = New-Object System.Collections.Generic.List[string]
    if ($versionJson.PSObject.Properties.Name -contains "CodexVersionJarIds") {
        foreach ($jarVersionId in @($versionJson.CodexVersionJarIds)) {
            $candidateJarVersionIds.Add([string]$jarVersionId)
        }
    }
    if ($versionJson.PSObject.Properties.Name -contains "inheritsFrom") {
        $candidateJarVersionIds.Add([string]$versionJson.inheritsFrom)
    }
    foreach ($jarVersionId in $candidateJarVersionIds) {
        if ([string]::IsNullOrWhiteSpace([string]$jarVersionId)) {
            continue
        }
        $inheritedJar = Join-Path $versionsRoot (Join-Path ([string]$jarVersionId) "$jarVersionId.jar")
        if (Test-Path -LiteralPath $inheritedJar) {
            $realVersionJar = $inheritedJar
            break
        }
    }
}
...
function Get-JavaMajor([string]$JavaHome) {
    ...
    try {
        ...
        $match = [regex]::Match($versionText, '(?:version\s+"|openjdk\s+)(?:1\.)?(\d+)')
...
function Select-TestJavaHome {
    ...
    $requiredMajor = if ($major -gt 0) { $major } else { $FallbackMajor }
...
$projectJavaProperties = Get-GradlePropertyMap -ProjectPath $project
$projectClientJavaMajor = 17
foreach ($key in @('client_java_version', 'java_version')) {
    $candidateMajor = 0
    if ([int]::TryParse([string]$projectJavaProperties[$key], [ref]$candidateMajor) -and $candidateMajor -gt 0) {
        $projectClientJavaMajor = $candidateMajor
        break
    }
}
$selectedJavaHome = Select-TestJavaHome -VersionJson $versionJson -FallbackMajor $projectClientJavaMajor
...
# Java 8 reads @argfiles through the active Windows code page. Keep the
# response-file path ASCII for legacy projects that themselves live below a
# non-ASCII workspace path; the already-normalized test version directory is
# safe on every supported client.
if ($projectClientJavaMajor -le 8) {
    $argFile = Join-Path $versionDir "codex-runclient.args"
}
...
} elseif (-not [string]::IsNullOrWhiteSpace([string]$versionJson.minecraftArguments)) {
    # Pre-1.13 version JSON stores game arguments in one legacy template rather
    # than the modern `arguments.game` array. Expanding this template preserves
    # Forge's required FMLTweaker and lets LaunchWrapper remap the 1.12 client
    # classes before loading net.minecraft.client.Minecraft.
    $legacyArguments = [string]$versionJson.minecraftArguments
    foreach ($variable in $launchVariables.GetEnumerator()) {
        $legacyArguments = $legacyArguments.Replace('${' + $variable.Key + '}', [string]$variable.Value)
    }
    $gameArgs = @($legacyArguments -split '\s+' | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
} else {
    $gameArgs = @(
...
$startProcessArgs = @{
    FilePath = $javaExe
    # Java response files were introduced after Java 8. Legacy Forge clients
    # must receive the expanded, ASCII-normalized argument array directly.
    ArgumentList = if ($projectClientJavaMajor -le 8) { @($jvmArgs + $gameArgs) } else { @("@$argFile") }
