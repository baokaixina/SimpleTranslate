 ...
 *
 * <p>Serializes each {@link ITextComponent}, projects every natural-language run to
 * an ordered top-level ITextComponent slot, and keeps opaque visuals, custom-font
 ...
 * requires the same top-level slot count and a parseable ITextComponent at every
...
public final class JsonPassthroughPipeline {
    ...
    static boolean cacheTemplateMatchesSourceMarkers(String sourceJson, String cacheTemplate) {
        if (blank(sourceJson) || blank(cacheTemplate)) {
            return false;
        }
        try {
            return ComponentJsonNumberNormalizer.hasSameDynamicMarkerDomain(
                    new com.google.gson.JsonParser().parse(sourceJson), new com.google.gson.JsonParser().parse(cacheTemplate));
    ...
    private static boolean cacheTemplateContainsDynamicMarkers(String cacheTemplate) {
        if (blank(cacheTemplate)) {
            return false;
        }
        try {
            return ComponentJsonNumberNormalizer.containsDynamicMarker(
                    new com.google.gson.JsonParser().parse(cacheTemplate));
        } catch (RuntimeException ignored) {
            return true;
        }
    }
     ...
     */
    private static List<String> extractDynamicValues(List<ITextComponent> originals) {
        ...
        try {
            JsonElement root = new com.google.gson.JsonParser().parse(sourceJson);
    ...
    private static String serializeComponentsRaw(List<ITextComponent> components) {
        if (components == null) {
            return null;
        }
        try {
            ...
            for (ITextComponent component : components) {
                String json = ComponentJsonCompat.toJson(
                        component == null ? com.yourname.simpletranslate.core.LegacyComponentFactory.empty() : component);
    ...
    public static ComponentListTranslationResult translateComponents(
        ...
        return SafeTranslate.guard(() -> {
            ...
            if (containsBlacklistedEntry(components)) {
                return new ComponentListTranslationResult(components, false, false);
            }
            ...
            if (ComponentJsonLayoutGuard.shouldKeepLayoutCriticalHudOriginal(
                    surface, sourceJson, targetLanguage)) {
                return new ComponentListTranslationResult(components, false, false);
            }
            String cacheKey = buildCacheKey(surface, sourceJson, context, role, sourceLanguage, targetLanguage);
            List<ITextComponent> cached = restoreCachedComponents(
    ...
    /**
     * Materializes a locally merged ITextComponent result into the ordinary exact
     ...
     * returned by a successful ITextComponent JSON request.  The stored value is
    ...
    public static void cacheResolvedComponents(
            List<ITextComponent> originals, List<ITextComponent> translated,
        ...
        if (!ModConfig.GLOBAL_ENABLED.get() || !ModConfig.CACHE_ENABLED.get()
                ...
                || !SimpleTranslateForge1122.isRuntimeRevisionCurrent(runtimeRevision)) {
            return;
        }
        try {
            String sourceLanguage = ModConfig.SOURCE_LANGUAGE.get();
            ...
            List<ITextComponent> replayed = deserializeComponents(response, originals);
            if (replayed == null) {
                return;
            }
    ...
    private static String buildCacheTemplateFromResolvedComponents(
            String sourceJson, List<ITextComponent> originals,
            List<ITextComponent> translated, String targetLanguage) {
        if (blank(sourceJson)
                || originals == null || originals.isEmpty()
                || translated == null || translated.size() != originals.size()) {
            return null;
        ...
        try {
            ...
            List<String> translatedSlots = liveProjection.alignedTranslatedSlotTexts(
                    new com.google.gson.JsonParser().parse(translatedJson));
            ...
            JsonArray rebuilt = normalizedProjection.rebuildComponents(translatedSlots.stream()
                    .map(text -> (ITextComponent) com.yourname.simpletranslate.core.LegacyComponentFactory.literal(text))
    ...
    public static String serializeComponents(List<ITextComponent> components) {
     ...
     */
    public static String serializeProjectionSource(List<ITextComponent> components) {
        return serializeComponentsRaw(components);
    }

    /** Builds the same marker-free projection from a live, hover-sanitized tree. */
    private static List<ITextComponent> liveProjectionRows;
    ...
    public static ComponentVisualProjection projectLiveComponents(
            List<ITextComponent> components, String targetLanguage) {
    ...
    private static String serializeComponents(List<ITextComponent> components, boolean includeHoverEvents) {
        if (components == null) {
            return null;
        }
        try {
            ...
            SimpleTranslateForge1122.getLogger().warn("JSON passthrough: failed to serialize components", e);
            return null;
        }
    }

    private static boolean hasTranslatableText(List<ITextComponent> components, String targetLanguage) {
        for (ITextComponent component : components) {
            if (component != null && TranslationTextDetector.containsTranslatableText(
                    component.getUnformattedText(), 1, targetLanguage)) {
                return true;
            }
        }
        return false;
    ...
    private static String buildCacheKey(String surface, String sourceJson, String context, String role,
        ...
        normalizedContext = blank(normalizedContext)
                ? semanticContext
                : normalizedContext + "\n" + semanticContext;
    ...
    private static List<ITextComponent> restoreLegacyJsonCache(String surface, String sourceJson,
                                                          String currentKey, List<ITextComponent> originals) {
        if (!ModConfig.CACHE_ENABLED.get()) {
            return null;
        }
        TranslationCache cache = SimpleTranslateForge1122.getTranslationCache();
        if (cache == null) {
            return null;
        }
        ...
        List<ITextComponent> restored = deserializeComponents(cached, originals, null);
        if (restored == null) {
            cache.remove(legacyKey);
            cache.save();
            return null;
        }
        List<ITextComponent> visibleRestored = reattachOriginalHoverEvents(restored, originals);
    ...
    private static List<ITextComponent> restoreCachedComponents(
            String surface, String sourceJson, String context,
            String sourceLanguage, String targetLanguage,
            String cacheKey, List<ITextComponent> components) {
        List<ITextComponent> cached = restoreFromCache(cacheKey, sourceJson, components);
        // Old marker/style-wire generations stay inactive. Only the original
        // json.<surface> ITextComponent cache may migrate lazily when the current
    ...
    private static List<ITextComponent> restoreFromCache(
            String cacheKey, String sourceJson, List<ITextComponent> originals) {
        if (!ModConfig.CACHE_ENABLED.get()) {
            return null;
        }
        TranslationCache cache = SimpleTranslateForge1122.getTranslationCache();
        ...
        List<ITextComponent> restored = deserializeComponents(cached, originals);
        if (restored == null) {
            cache.remove(cacheKey);
            cache.save();
            return null;
    ...
    private static CompletableFuture<List<ITextComponent>> requestAsync(
            List<ITextComponent> components, String sourceJson, String surface,
            String role, String context, String cacheKey,
            String sourceLanguage, String targetLanguage) {
        TranslationManager manager = SimpleTranslateForge1122.getTranslationManager();
        ...
        long runtimeRevision = SimpleTranslateForge1122.getRuntimeRevision();
        long textContextRevision = TextContextMemory.revision();
        BatchItem item = new BatchItem(manager,
                Collections.unmodifiableList(new ArrayList<ITextComponent>(components)), sourceJson, surface,
        ...
        if (Surface.directBatchCandidate(surface) && blank(item.context())) {
            return ComponentJsonBatcher.enqueue(item);
        }
        return sendSingle(item);
    }

    static CompletableFuture<List<ITextComponent>> sendSingle(BatchItem item) {
        if (!isCurrent(item)) {
    ...
    public static String buildUserPayload(String sourceJson, String context) {
        ...
        // one top-level JSON array in the current Component projection wire.
        return blank(sourceJson) ? "[]" : sourceJson.trim();
    }

    /**
     * Pages and signs are multi-ITextComponent physical layouts whose lines form one
    ...
    static List<Integer> recoveryAtomicGroupSizesForTest(
        ...
        if (normalized.startsWith("book.") || normalized.startsWith("sign.")
            ...
            return Collections.singletonList(slotCount);
        }
        return ordinaryAtomicGroups == null ? Collections.<Integer>emptyList()
                : Collections.unmodifiableList(new ArrayList<Integer>(ordinaryAtomicGroups));
    }

    /**
     * Translates one complete semantic document through the approved ITextComponent
     * JSON protocol. The only response contract is exact top-level count plus a
     * parseable ITextComponent at every index. A retained player name, product name,
    ...
    private static CompletableFuture<String> translateComponentSlotsReliably(
        ...
        return item.manager().translateComponentJson(
                ...
                .thenCompose(response -> {
                    ...
                    // non-empty response whose JSON/count/ITextComponent structure is
    ...
    /**
     * Some ITextComponent arrays are visually fragmented but grammatically atomic.
    ...
    private static CompletableFuture<String> recoverComponentSlotsByPartition(
        ...
        List<Integer> leftGroups = Collections.unmodifiableList(new ArrayList<Integer>(
                atomicGroupSizes.subList(0, groupBoundary)));
        List<Integer> rightGroups = Collections.unmodifiableList(new ArrayList<Integer>(
        ...
        List<CompletableFuture<String>> requests = Arrays.asList(
        ...
        CompletableFuture<?>[] pending = requests.toArray(new CompletableFuture<?>[requests.size()]);
        return CompletableFuture.allOf(pending).thenApply(ignored -> {
            ...
            return combineComponentPartitionResponses(
                    responses, Arrays.asList(leftCount, rightCount), item.surface());
        });
    }

    /** Bisects only at already-validated top-level ITextComponent boundaries. */
    ...
    private static List<String> bisectComponentRecoveryPayload(
        ...
        try {
            JsonElement parsed = new com.google.gson.JsonParser().parse(
            ...
            for (int index = 0; index < expectedSlots; index++) {
                ...
                (index < midpoint ? left : right).add(com.yourname.simpletranslate.core.JsonCompat.deepCopy(element));
            }
            return Arrays.asList(left.toString(), right.toString());
        } catch (RuntimeException exception) {
            return Collections.emptyList();
        }
    }
    ...
    /** Recombines only exact ITextComponent partitions in original request order. */
    private static String combineComponentPartitionResponses(
        ...
        for (int index = 0; index < responses.size(); index++) {
            ...
            for (JsonElement element : partition) {
                combined.add(com.yourname.simpletranslate.core.JsonCompat.deepCopy(element));
            }
        }
    ...
    private static String componentPartitionRecoveryContext(
        ...
        try {
            JsonElement parsed = new com.google.gson.JsonParser().parse(
                    blank(promptContext) ? "{}" : promptContext);
            metadata = parsed.isJsonObject()
                    ? com.yourname.simpletranslate.core.JsonCompat.deepCopy(parsed.getAsJsonObject()) : new JsonObject();
        } catch (RuntimeException ignored) {
     ...
     * The user payload remains exactly the original top-level ITextComponent array.
     */
    private static String componentStructureAttemptContext(
        ...
        try {
            JsonElement parsed = new com.google.gson.JsonParser().parse(
                    blank(promptContext) ? "{}" : promptContext);
            metadata = parsed.isJsonObject() ? com.yourname.simpletranslate.core.JsonCompat.deepCopy(parsed.getAsJsonObject()) : new JsonObject();
        } catch (RuntimeException ignored) {
            metadata = new JsonObject();
        }
    ...
    private static JsonArray parseExactComponentArray(
        ...
        try {
            JsonElement parsed = new com.google.gson.JsonParser().parse(response == null ? "" : response.trim());
    ...
    private static List<ITextComponent> parseExactComponentList(
            String response, int expected, String surface, int offset) {
        ...
        try {
            List<ITextComponent> components = new ArrayList<>(array.size());
    ...
    private static CompletableFuture<List<ITextComponent>> finishRequest(
            List<ITextComponent> originals, String sourceJson, String response,
        ...
        try {
            ...
            List<ITextComponent> semanticResponse = parseExactComponentList(
            ...
            List<ITextComponent> accepted = acceptRestored(
    ...
    static List<ITextComponent> acceptRestored(
            List<ITextComponent> originals, String sourceJson, String response, List<ITextComponent> restored,
        ...
        List<ITextComponent> visibleRestored = reattachOriginalHoverEvents(restored, originals);
        if (containsBlacklistedEntry(originals)) {
            lane.finish(lease);
            return null;
        }
        if (containsBlacklistedEntry(visibleRestored)) {
            lane.fail(lease, FAILURE_RETRY_MS);
            RecoveryPolicy.recordRejected(cacheKey);
            return null;
        }
        if (ModConfig.TERM_AUTO_DETECT_ENABLED.get()) {
            com.yourname.simpletranslate.cache.TermDictionary dictionary =
                    SimpleTranslateForge1122.getTermDictionary();
            if (dictionary != null) dictionary.analyzeAndRecordTerms(plainText(originals));
        }
    ...
    public static List<ITextComponent> deserializeComponents(String response, List<ITextComponent> originals) {
    ...
    private static DecodedProjectedResponse decodeProjectedResponse(
            String response, List<ITextComponent> originals, String sourceJson, String surface,
            ComponentVisualProjection projection) {
        if (projection == null || blank(response)) {
            return null;
        }
        try {
            JsonElement root = new com.google.gson.JsonParser().parse(response.trim());
            ...
            List<ITextComponent> restored = finalizeTranslatedTree(
                    com.yourname.simpletranslate.core.JsonCompat.deepCopy(rebuilt), originals, surface, response);
    ...
    private static List<ITextComponent> deserializeComponents(
            String response, List<ITextComponent> originals, String surface) {
        try {
            JsonElement root = new com.google.gson.JsonParser().parse(response.trim());
            if (!root.isJsonArray()) {
    ...
    static List<ITextComponent> finalizeTranslatedTree(
            JsonArray array, List<ITextComponent> originals, String surface, String response) {
        ...
        List<ITextComponent> result = new ArrayList<>(expected);
        ...
        return Collections.unmodifiableList(new ArrayList<ITextComponent>(result));
    }
     ...
     */
    public static boolean isLayoutCriticalFont(String font) {
     ...
     */    private static JsonElement normalizeComponentJson(JsonElement element) {
        ...
        JsonObject normalized = com.yourname.simpletranslate.core.JsonCompat.deepCopy(element.getAsJsonObject());
    ...
    private static void logDeserializationFailure(
            String surface, String reason, List<ITextComponent> originals, int actual, String response) {
        if (blank(surface)) {
            return;
        }
        SimpleTranslateForge1122.getLogger().warn(
    ...
    public static List<ITextComponent> reattachOriginalHoverEventsForRender(
            List<ITextComponent> translated, List<ITextComponent> originals) {
        return reattachOriginalHoverEvents(translated, originals);
    }

    private static List<ITextComponent> reattachOriginalHoverEvents(List<ITextComponent> translated, List<ITextComponent> originals) {
        if (translated == null || translated.isEmpty() || originals == null || originals.size() != translated.size()) {
            return translated;
        }
        List<ITextComponent> result = new ArrayList<>(translated.size());
        boolean changed = false;
        for (int i = 0; i < translated.size(); i++) {
            ITextComponent translatedComponent = translated.get(i);
            ITextComponent originalComponent = originals.get(i);
            ITextComponent restored = reattachOriginalHoverEvents(translatedComponent, originalComponent);
            result.add(restored);
            changed |= restored != translatedComponent;
        }
        return changed ? Collections.unmodifiableList(new ArrayList<ITextComponent>(result)) : translated;
    }

    private static ITextComponent reattachOriginalHoverEvents(ITextComponent translated, ITextComponent original) {
        if (translated == null || original == null) {
            return translated;
        }
        try {
            JsonElement translatedJson = new com.google.gson.JsonParser().parse(ComponentJsonCompat.toJson(translated));
            JsonElement originalJson = new com.google.gson.JsonParser().parse(ComponentJsonCompat.toJson(original));
            translatedJson = objectTargetForHoverCopy(translatedJson, originalJson);
            copyOriginalHoverEvents(originalJson, translatedJson);
            ITextComponent restored = ComponentJsonCompat.fromJson(translatedJson);
    ...
    private static JsonElement objectTargetForHoverCopy(JsonElement translated, JsonElement original) {
        ...
        if (translated.isJsonPrimitive()) {
            ...
            wrapped.add("extra", com.yourname.simpletranslate.core.JsonCompat.deepCopy(translated));
        }
        return wrapped;
    }
    ...
    private static void copyOriginalHoverEvents(JsonElement original, JsonElement translated) {
        ...
        if (originalHoverEvent != null) {
            translatedObject.add(hoverEventKey(originalObject), com.yourname.simpletranslate.core.JsonCompat.deepCopy(originalHoverEvent));
    ...
    private static void cacheSuccessfulResponse(
            ...
            List<ITextComponent> originals, List<ITextComponent> restored,
    ...
    private static String plainText(List<ITextComponent> components) {
        if (components == null || components.isEmpty()) {
            return "";
        }
        ...
        for (int i = 0; i < components.size(); i++) {
            ...
            if (component != null) {
                text.append(component.getUnformattedText());
            }
        }
        return text.toString();
    }

    private static boolean containsBlacklistedEntry(List<ITextComponent> components) {
        TranslationBlacklist blacklist = SimpleTranslateForge1122.getTranslationBlacklist();
        return blacklist != null && blacklist.containsBlacklistedEntry(plainText(components));
    }
     ...
     * strict semantic ITextComponent array; this is context metadata only.
     */
    public static String semanticPromptSourceBlock(List<ITextComponent> components) {
    ...
    public static String semanticPromptSourceShape(List<ITextComponent> components) {
        String readable = semanticPromptSourceBlock(components);
        return blank(readable) ? ""
                : ComponentJsonNumberNormalizer.maskPromptDynamicNumbers(readable);
    }

    private static final class DecodedProjectedResponse {
        private final String cacheTemplate;
        private final List<ITextComponent> components;
        private DecodedProjectedResponse(String cacheTemplate, List<ITextComponent> components) {
            this.cacheTemplate = cacheTemplate;
            this.components = components;
        }
        private String cacheTemplate() { return cacheTemplate; }
        private List<ITextComponent> components() { return components; }
    }

    static final class BatchItem {
        private final TranslationManager manager;
        private final List<ITextComponent> originals;
        private final String sourceJson;
        private final String surface;
        private final String role;
        private final String context;
        private final String cacheKey;
        private final TranslationLane lane;
        private final TranslationLane.Lease lease;
        private final long runtimeRevision;
        private final long textContextRevision;
        private final String sourceLanguage;
        private final String targetLanguage;
        private final CompletableFuture<List<ITextComponent>> future;
        BatchItem(TranslationManager manager, List<ITextComponent> originals, String sourceJson,
                  String surface, String role, String context, String cacheKey, TranslationLane lane,
                  TranslationLane.Lease lease, long runtimeRevision, long textContextRevision,
                  String sourceLanguage, String targetLanguage,
                  CompletableFuture<List<ITextComponent>> future) {
            ...
            this.textContextRevision = textContextRevision;
            this.sourceLanguage = sourceLanguage;
            this.targetLanguage = targetLanguage;
            this.future = future;
        }
        TranslationManager manager() { return manager; }
        List<ITextComponent> originals() { return originals; }
        ...
        TranslationLane lane() { return lane; }
        TranslationLane.Lease lease() { return lease; }
        long runtimeRevision() { return runtimeRevision; }
        long textContextRevision() { return textContextRevision; }
        String sourceLanguage() { return sourceLanguage; }
        String targetLanguage() { return targetLanguage; }
        CompletableFuture<List<ITextComponent>> future() { return future; }
    }

    private static boolean blank(String value) { return value == null || value.trim().isEmpty(); }

    static boolean isCurrent(BatchItem item) {
        return item != null
                && SimpleTranslateForge1122.isRuntimeRevisionCurrent(item.runtimeRevision())
