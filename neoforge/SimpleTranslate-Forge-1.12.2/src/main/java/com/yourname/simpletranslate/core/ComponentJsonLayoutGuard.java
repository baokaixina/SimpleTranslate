...
public final class ComponentJsonLayoutGuard {
    ...
    public static boolean shouldKeepLayoutCriticalHudOriginal(String surface, String sourceJson,
        ...
        if (!isCjkTargetLanguage(targetLanguage) || sourceJson == null || sourceJson.trim().isEmpty()) {
            return false;
        }
        try {
    ...
    public static boolean isLayoutCriticalHudTree(JsonElement root) {
        ...
        return (stats.hasKnownLayoutFont && stats.privateUseGlyphs > 0)
                ...
                // 1.12.2 has no per-component font field. Resource packs still
                // encode positioned HUD streams as dense private-use glyphs in
                // the active global font, so use the glyph stream itself.
                || stats.totalPrivateUseGlyphs >= 4;
    }
    ...
    public static boolean satisfiesInPlaceLayoutContract(String translationJson, String sourceJson) {
        ...
        if (translationJson == null || translationJson.trim().isEmpty()) {
            return false;
        }
        try {
    ...
    public static boolean isLayoutCriticalFont(String font) {
        if (font == null || font.trim().isEmpty() || "minecraft:default".equals(font)) {
            return false;
        }
    ...
    private static void collectLayoutFontStats(JsonElement element, boolean inheritedLayoutFont,
        ...
        if (!element.isJsonObject()) {
            if (element.isJsonPrimitive() && element.getAsJsonPrimitive().isString()) {
                stats.totalPrivateUseGlyphs += countPrivateUseCodepoints(element.getAsString());
            }
        ...
        stats.totalPrivateUseGlyphs += puaCount;
        ...
        boolean layoutFont = knownLayoutFont
                || puaHeavyCustomFont
                || (inheritedLayoutFont && !hasDefaultFont(object));
        if (layoutFont && font != null && !font.trim().isEmpty() && !"minecraft:default".equals(font)) {
            stats.layoutFontIds.add(font);
        } else if (layoutFont && (font == null || font.trim().isEmpty())) {
    ...
    private static boolean hasCustomFont(JsonObject object) {
        String font = readFont(object);
        return font != null && !font.trim().isEmpty() && !"minecraft:default".equals(font);
    }
