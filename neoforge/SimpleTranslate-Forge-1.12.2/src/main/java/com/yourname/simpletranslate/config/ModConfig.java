...
public class ModConfig {
    ...
    private static final Logger LOGGER = LogManager.getLogger("SimpleTranslate/Config-1.12.2");
    ...
    public static final ConfigValue<String> SHORTCUT_TRANSLATE_GUI = stringValue(
            "shortcuts.translateGui", KeyChord.keyboard(Keyboard.KEY_K).serialize());
    public static final ConfigValue<String> SHORTCUT_TRANSLATE_TOOLTIP = stringValue(
            "shortcuts.translateTooltip", KeyChord.keyboard(Keyboard.KEY_V).serialize());
    public static final ConfigValue<String> SHORTCUT_SIGN_SELECT = stringValue(
            "shortcuts.signSelect", KeyChord.keyboard(Keyboard.KEY_G).serialize());
    public static final ConfigValue<String> SHORTCUT_SIGN_SUBMIT = stringValue(
            "shortcuts.signSubmit", KeyChord.keyboard(Keyboard.KEY_H).serialize());
    ...
    public static ConfigValue<String> getShortcutChord(ShortcutAction action) {
        switch (action) {
            case TOGGLE_GLOBAL_TRANSLATION: return SHORTCUT_TOGGLE_GLOBAL_TRANSLATION;
            case TOGGLE_CHAT_MODE: return SHORTCUT_TOGGLE_CHAT_MODE;
            case TRANSLATE_GUI: return SHORTCUT_TRANSLATE_GUI;
            case TRANSLATE_TOOLTIP: return SHORTCUT_TRANSLATE_TOOLTIP;
            case SIGN_SELECT: return SHORTCUT_SIGN_SELECT;
            case SIGN_SUBMIT: return SHORTCUT_SIGN_SUBMIT;
            default: return SHORTCUT_TOGGLE_GLOBAL_TRANSLATION;
        }
    }

    public static ConfigValue<String> getHoldOriginalChord(HoldOriginalFeature feature) {
        switch (feature) {
            case CHAT: return HOLD_ORIGINAL_CHORD_CHAT;
            case TOOLTIP_ITEM: return HOLD_ORIGINAL_CHORD_TOOLTIP_ITEM;
            case TOOLTIP_HOVER: return HOLD_ORIGINAL_CHORD_TOOLTIP_HOVER;
            case BOOK: return HOLD_ORIGINAL_CHORD_BOOK;
            case SIGN: return HOLD_ORIGINAL_CHORD_SIGN;
            case ADVANCEMENT: return HOLD_ORIGINAL_CHORD_ADVANCEMENT;
            case ENTITY_NAME: return HOLD_ORIGINAL_CHORD_ENTITY_NAME;
            case SCOREBOARD: return HOLD_ORIGINAL_CHORD_SCOREBOARD;
            case BOSSBAR: return HOLD_ORIGINAL_CHORD_BOSSBAR;
            case TITLE: return HOLD_ORIGINAL_CHORD_TITLE;
            case ACTIONBAR: return HOLD_ORIGINAL_CHORD_ACTIONBAR;
            case GUI: return HOLD_ORIGINAL_CHORD_GUI;
            default: return HOLD_ORIGINAL_CHORD_GUI;
        }
    }
    ...
    public static String normalizeDeepSeekModelId(String modelId) {
        ...
        if ("deepseekv4pro".equals(compact) || "deepseek-v4pro".equals(compact)
                || "v4pro".equals(compact) || "pro".equals(compact)
                || "reasoner".equals(compact) || "deepseekreasoner".equals(compact)) {
            return DeepSeekModelPreset.V4_PRO.getModelId();
        }
        return DeepSeekModelPreset.V4_FLASH.getModelId();
    }
    ...
    public static String normalizeModelId(String modelId) {
        ...
        return normalized.isEmpty() ? format.getDefaultModel() : normalized;
    }
    ...
    public static synchronized void load() {
        ...
        try {
            ...
            JsonObject root = new com.google.gson.JsonParser().parse(raw).getAsJsonObject();
            migrateLegacyKeys(root);
            loadedRoot = com.yourname.simpletranslate.core.JsonCompat.deepCopy(root);
            ...
            LOGGER.error("Config file is not valid JSON; backing it up before regenerating", e);
    ...
    private static void backupCorruptConfigFile() {
        try {
            if (configFile != null && Files.exists(configFile)) {
                ...
                LOGGER.warn("Backed up unreadable config to {}", backup);
            }
    ...
    public static synchronized void save() {
        ...
        try {
            Files.createDirectories(configFile.getParent());
            JsonObject root = loadedRoot == null ? new JsonObject() : com.yourname.simpletranslate.core.JsonCompat.deepCopy(loadedRoot);
            for (Map.Entry<String, ConfigValue<?>> entry : VALUES.entrySet()) {
    ...
    private static void copyLegacyKey(JsonObject root, String oldKey, String newKey) {
        ...
        root.add(newKey, com.yourname.simpletranslate.core.JsonCompat.deepCopy(root.get(oldKey)));
    }
