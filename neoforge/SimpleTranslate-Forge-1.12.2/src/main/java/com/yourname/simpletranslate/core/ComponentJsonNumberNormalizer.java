...
public final class ComponentJsonNumberNormalizer {
    ...
    private static boolean isDynamicValue(String text, int start, int end) {
        ...
        String prefix = line.substring(0, Math.min(localStart, line.length())).trim();
        String suffix = line.substring(Math.min(localEnd, line.length())).trim();
        ...
        boolean valueAtStatusEdge = prefix.isEmpty() || isDataOnlyRemainder(suffix);
