 ...
 *
 * <p>API surface verified with {@code javap -p -s} against
 * Iceberg-1.21.3-neoforge-1.2.10 (the exact NeoForge artifact for Minecraft
 * 1.21.3; no Iceberg build was ever released for Minecraft 1.21.2, so this is
 * the authoritative artifact for the 1.21.2/1.21.3 era): the event class is
 * {@code com.anthonyhilyard.iceberg.events.client.RenderTooltipEvents} and its
 * {@code GATHER} is Iceberg's own public
 * {@code com.anthonyhilyard.iceberg.events.Event} with a public
 * {@code register(T)}; {@code Gather.onGather} takes
 * {@code (ItemStack, int, int, List, int, int)} and {@code GatherResult} has a
 * public {@code (InteractionResult, int, List)} constructor. The actual
 * translation runs through
