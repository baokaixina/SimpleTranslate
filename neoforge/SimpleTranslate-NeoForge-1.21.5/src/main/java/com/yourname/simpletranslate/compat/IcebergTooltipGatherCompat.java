 ...
 *
 * <p>API surface verified against the Iceberg-1.21.4-neoforge-1.2.13 jar (its
 * neoforge.mods.toml declares Minecraft [1.21.3,), so it is the exact Iceberg
 * build loadable on 1.21.5): the
 * event class lives at {@code com.anthonyhilyard.iceberg.events.client.RenderTooltipEvents}
 * and its {@code GATHER} is Iceberg's own public {@code com.anthonyhilyard.iceberg.events.Event},
 * with {@code Gather#onGather(ItemStack,int,int,List,int,int)} and a public
 * {@code GatherResult(InteractionResult,int,List)} record constructor — the same
 * shape as the Fabric 1.3.x series. The actual translation runs through
